...
def check_pooltype(pooltype):
    ...
    for tno, trec in enumerate(test_cases):
        ...
        attrs = {}
        if "dilations" in trec:
            attrs["dilations"] = trec["dilations"]  # type: ignore
        if "pads" in trec:
            attrs["pads"] = trec["pads"]  # type: ignore
        if "strides" in trec:
            attrs["strides"] = trec["strides"]  # type: ignore
        if "auto_pad" in trec:
            attrs["auto_pad"] = trec["auto_pad"]  # type: ignore
        if "kernel_shape" in trec:
            attrs["kernel_shape"] = trec["kernel_shape"]  # type: ignore
        if "ceil_mode" in trec:
            attrs["ceil_mode"] = trec["ceil_mode"]  # type: ignore
        op_info = {
            "name": op_name,
            "optype": pooltype,
