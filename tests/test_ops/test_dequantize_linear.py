...
import pytest
import numpy as np
from ttsim.ops.op import DequantizeLinearOp
...
def test_dequantize_linear_basic_uint8():
    ...
    op_obj = DequantizeLinearOp(op_info)
...
@pytest.mark.unit
@pytest.mark.opunit
def test_dequantize_linear_factory():
    """Test that DequantizeLinear operation can be created through SimOpFactory"""
    print("\n=== Testing DequantizeLinear Factory Creation ===")

    from ttsim.ops.op import SimOpFactory

    # Test factory creation
    dequantize_class = SimOpFactory('DequantizeLinear')
    assert dequantize_class == DequantizeLinearOp

    # Create instance through factory
    op_info = {
        'name': 'factory_test_dequantize_linear',
        'optype': 'DequantizeLinear',
        'inList': ['input', 'scale', 'zero_point'],
        'outList': ['output'],
        'attrs': {'axis': 1},
    ...
    op_obj = dequantize_class(op_info)
    assert isinstance(op_obj, DequantizeLinearOp)
    assert op_obj.optype == 'DequantizeLinear'
    assert op_obj.name == 'factory_test_dequantize_linear'
    assert op_obj.axis == 1

    print("✓ DequantizeLinear factory creation test passed")
...
def test_dequantize_linear_quantization_workflow():
    ...
    print(f"  Performance: {perf_stats['instrs']['convert']} conversions, {perf_stats['instrs']['sub']} subtractions, {perf_stats['instrs']['mul']} multiplications")


if __name__ == '__main__':
    # Run all tests manually
    test_dequantize_linear_factory()
