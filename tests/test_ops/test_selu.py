...
import numpy as np
import pytest
from ttsim.ops.op import SimOpFactory, SeluOp
...
class TestSelu:
    ...
    def test_factory_integration(self):
        """Test that SELU is properly integrated into SimOpFactory"""
        opcls = SimOpFactory('Selu')
        assert opcls == SeluOp
    ...
    def test_selu_invalid_tensor_shape(self):
        ...
        with pytest.raises(AssertionError, match="SELU input must be at least 1D"):
    ...
    def test_selu_self_normalizing_properties(self):
        ...
        assert exp_ops < scale_ops  # Most values should be positive in self-normalizing networks
        assert exp_ops == total_elements // 2  # Estimated negative values
        assert scale_ops == total_elements     # Scaling applied to all values
