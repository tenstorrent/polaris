...
class TestGelu:
    ...
    def test_factory_integration(self):
        """Test that GELU is registered in the descriptor registry"""
        assert get_opdesc_registry().has_shape_inference_function('Gelu')
    ...
    def test_gelu_default_variant(self):
        ...
        assert 'exp' in perf_stats['instrs']
    ...
    def test_gelu_invalid_tensor_shape(self):
        ...
        # Descriptor accepts scalar; ensure perf still runs
        perf_stats = op.get_perf_counts(inT, outT)
        assert perf_stats is not None
    ...
    def test_gelu_variant_differences(self):
        ...
        op_default = SimOp(op_info_default)
        ...
        op_tanh = SimOp(op_info_tanh)
        perf_stats_tanh = op_tanh.get_perf_counts(inT, outT)

        # Variants should have different instruction profiles
        # Both may have 'exp' now (default has it explicitly, tanh via gelu_instr_counts)
        # but their counts should differ
        assert perf_stats_default['instrs'] != perf_stats_tanh['instrs']
