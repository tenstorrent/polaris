...
import pytest
import numpy as np
from ttsim.ops.op import DynamicQuantizeLinearOp
...
def test_dynamic_quantize_linear_basic():
    ...
    op_obj = DynamicQuantizeLinearOp(op_info)
...
def test_dynamic_quantize_linear_factory():
    """Test that DynamicQuantizeLinear operation can be created through SimOpFactory"""
    print("\n=== Testing DynamicQuantizeLinear Factory Creation ===")

    from ttsim.ops.op import SimOpFactory

    # Test factory creation
    dynamic_quantize_class = SimOpFactory('DynamicQuantizeLinear')
    assert dynamic_quantize_class == DynamicQuantizeLinearOp

    # Create instance through factory
    op_info = {
        'name': 'factory_test_dynamic_quantize',
        'optype': 'DynamicQuantizeLinear',
        'inList': ['input'],
        'outList': ['output', 'scale', 'zero_point'],
        'attrs': {},
    ...
    op_obj = dynamic_quantize_class(op_info)
    assert isinstance(op_obj, DynamicQuantizeLinearOp)
    assert op_obj.optype == 'DynamicQuantizeLinear'
    assert op_obj.name == 'factory_test_dynamic_quantize'

    print("✓ DynamicQuantizeLinear factory creation test passed")
