...
class TestShrink:
    ...
    def test_factory_integration(self):
        """Basic sanity test for Shrink creation via SimOp."""
        op_info = {
            'name': 'test_shrink_factory',
            'optype': 'Shrink',
            'inList': ['input'],
            'outList': ['output'],
            'attrs': {'lambd': 0.5, 'bias': 0.0},
        }
        op = SimOp(op_info)
        assert op.optype == 'Shrink'
    ...
    def test_shrink_invalid_tensor_shape(self):
        ...
        # Legacy Shrink restricted to 1D+. New desc handles scalar.
