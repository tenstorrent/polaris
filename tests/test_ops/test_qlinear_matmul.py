...
def test_qlinear_matmul_invalid_inputs():
    ...
    try:
        ...
        with pytest.raises((AssertionError, ValueError), match="Input x must be quantized type"):
            op_obj.get_perf_counts(input_tensors, output_tensors)
    except (AssertionError, ValueError) as e:
...
def test_qlinear_matmul_factory():
    """Test that QLinearMatMul operation is registered in the op mapping"""
    print("\n=== Testing QLinearMatMul Factory Creation ===")
    from ttsim.ops.desc.registry import get_opdesc_registry
    assert get_opdesc_registry().has_shape_inference_function('QLinearMatMul')
