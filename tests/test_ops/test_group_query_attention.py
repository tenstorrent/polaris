...
def create_group_query_attention_test_tensors(batch_size=2, seq_len_q=8, seq_len_k=6, num_heads=8,
    ...
    if use_past:
        ...
        # Past key/value should have kv_num_heads at dim 1 per ONNX spec
        past_key = F._from_shape('past_key', [batch_size, kv_num_heads, past_seq_len, head_dim], np_dtype=np.dtype('float32'))
        past_value = F._from_shape('past_value', [batch_size, kv_num_heads, past_seq_len, head_dim], np_dtype=np.dtype('float32'))
    ...
    if use_past and past_key is not None:
        total_seq_len += past_key.shape[2]  # seq_len is at index 2 now
...
class TestGroupQueryAttention:
    ...
    def test_factory_integration(self):
        """Sanity: GroupQueryAttentionOp class exists"""
        assert get_opdesc_registry().has_shape_inference_function('GroupQueryAttention')
    ...
    def test_missing_num_heads(self):
        ...
        # Need dummy tensors for get_perf_counts to trigger validation
        inT, outT, _ = create_group_query_attention_test_tensors(num_heads=8, kv_num_heads=4)

        with pytest.raises(ValueError, match="q_num_heads attribute is required"): # Expect q_num_heads check failure
            SimOp(op_info).get_perf_counts(inT, outT)
    ...
    def test_missing_kv_num_heads(self):
        ...
        # This fails because without kv_num_heads, it defaults to num_heads (8),
        # causing head_dim mismatch between Q (512/8=64) and K (256/8=32)
        with pytest.raises(ValueError, match="Q head_size .* must match K head_size"):
            SimOp(op_info).get_perf_counts(inT, outT)
    ...
    def test_invalid_group_size(self):
        ...
        # Use matching shapes for tensor creation to pass shape checks until logic check
        inT, outT, _ = create_group_query_attention_test_tensors(num_heads=7, kv_num_heads=3)
    ...
    def test_inconsistent_batch_sizes(self):
        ...
        with pytest.raises(ValueError, match="Batch size mismatch between Q and K"):
    ...
    def test_inconsistent_sequence_lengths(self):
        ...
        with pytest.raises(ValueError, match="K and V sequence lengths must match"):
            op.get_perf_counts(inT, outT)
    ...
    def test_invalid_hidden_sizes(self):
        ...
        with pytest.raises(ValueError, match="q_hidden_size .* not divisible by q_num_heads"):
            op.get_perf_counts(inT, outT)
