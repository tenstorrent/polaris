...
"""
Comprehensive tests for the ArgMin op.

ArgMin returns the indices of the minimum values along a given axis.
Registration: ARITY_1->1, r0_func shape inference, compute_argmin.
...
from ttsim.ops.desc.data_compute import compute_argmin
...
# Helper
# ---------------------------------------------------------------------------
def _run_argmin(data, axis=0, keepdims=1, select_last_index=0, tag="argmin"):
    """Run ArgMin through SimOp and return (actual_data, expected_data, oT)."""
    ...
    o_tensors[0].data = compute_argmin(i_tensors, op)
    ...
    if select_last_index:
        ...
        idx_rev = np.argmin(X_rev, axis=ax, keepdims=bool(keepdims))
        expected = (X.shape[ax] - 1 - idx_rev).astype(np.int64)
    else:
        expected = np.argmin(X, axis=ax, keepdims=bool(keepdims)).astype(np.int64)
...
class TestArgMinShape:
    ...
    def test_output_shape(self, shape, axis, keepdims, expected_shape, tid):
        ...
        _, _, oT = _run_argmin(data, axis=axis, keepdims=keepdims, tag=f"shape_{tid}")
        assert (
    ...
    def test_output_dtype_int64(self):
        ...
        actual, _, oT = _run_argmin(data, axis=0, tag="dtype_check")
...
# 3. Edge cases
# ===================================================================
class TestArgMinEdgeCases:
    @pytest.mark.unit
    @pytest.mark.opunit
    def test_single_element(self):
        """Single element tensor -> index is 0."""
        data = np.array([42.0])
        actual, expected, _ = _run_argmin(data, axis=0, keepdims=0, tag="single")
        assert actual == 0
    ...
    def test_all_same_values(self):
        ...
        actual, expected, _ = _run_argmin(data, axis=1, keepdims=0, tag="same_vals")
    ...
    def test_negative_axis(self):
        ...
        actual_neg, _, _ = _run_argmin(data, axis=-1, keepdims=1, tag="neg_ax")
        actual_pos, _, _ = _run_argmin(data, axis=2, keepdims=1, tag="pos_ax")
    ...
    def test_min_at_beginning(self):
        ...
        data = np.array([[1.0, 10.0, 20.0, 30.0]])
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="min_begin")
        assert actual[0] == 0
    ...
    def test_min_at_end(self):
        ...
        data = np.array([[10.0, 20.0, 30.0, 1.0]])
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="min_end")
        assert actual[0] == 3

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_negative_values(self):
        """All negative values — argmin finds most negative."""
        data = np.array([[-1.0, -5.0, -3.0, -2.0]])
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="neg_vals")
        assert actual[0] == 1  # -5.0 is the min

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_neg_inf_is_min(self):
        """Negative infinity is always the argmin."""
        data = np.array([[1.0, -np.inf, -100.0, -999.0]])
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="neg_inf_min")
        assert actual[0] == 1
    ...
    def test_large_tensor(self):
        ...
        actual, expected, _ = _run_argmin(data, axis=2, keepdims=0, tag="large")
        np.testing.assert_array_equal(actual, expected)

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_select_last_index(self):
        """select_last_index=1 returns index of last occurrence of min."""
        data = np.array([[2.0, 3.0, 2.0, 5.0]])
        actual, _, _ = _run_argmin(
            data, axis=1, keepdims=0, select_last_index=1, tag="last_idx"
        )
        assert actual[0] == 2  # last occurrence of 2.0
    ...
    def test_select_first_index_default(self):
        ...
        assert actual[0] == 0  # first occurrence of 2.0


# ===================================================================
# 4. Precision tests with known values
# ===================================================================
class TestArgMinPrecision:
    @pytest.mark.unit
    @pytest.mark.opunit
    def test_identity_matrix_rows(self):
        """Identity matrix — argmin along axis=1 returns off-diagonal (first zero)."""
        data = np.eye(4, dtype=np.float64)
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="eye_rows")
        # For row 0: [1,0,0,0] -> argmin=1; row 1: [0,1,0,0] -> argmin=0; etc.
        expected = np.array([1, 0, 0, 0], dtype=np.int64)
        np.testing.assert_array_equal(actual, expected)

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_identity_matrix_cols(self):
        """Identity matrix — argmin along axis=0 returns off-diagonal (first zero)."""
        data = np.eye(4, dtype=np.float64)
        actual, _, _ = _run_argmin(data, axis=0, keepdims=0, tag="eye_cols")
        # For col 0: [1,0,0,0] -> argmin=1; col 1: [0,1,0,0] -> argmin=0; etc.
        expected = np.array([1, 0, 0, 0], dtype=np.int64)
        np.testing.assert_array_equal(actual, expected)
    ...
    def test_ascending_sequence(self):
        """Ascending sequence -> argmin is first element."""
        data = np.arange(10, dtype=np.float64).reshape(1, 10)
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="asc")
        assert actual[0] == 0

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_descending_sequence(self):
        """Descending sequence -> argmin is last element."""
        data = np.arange(10, 0, -1, dtype=np.float64).reshape(1, 10)
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="desc")
        assert actual[0] == 9

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_one_hot_rows_argmin(self):
        """Rows with a single negative — argmin gives that index."""
        data = np.zeros((3, 5), dtype=np.float64)
        data[0, 2] = -1.0
        data[1, 4] = -1.0
        data[2, 0] = -1.0
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="onehot_min")
    ...
    def test_keepdims_shape_matches(self):
        ...
        _, _, oT_kd = _run_argmin(data, axis=1, keepdims=1, tag="kd_yes")
        _, _, oT_nokd = _run_argmin(data, axis=1, keepdims=0, tag="kd_no")
...
class TestArgMinProperties:
    ...
    @pytest.mark.unit
    @pytest.mark.opunit
    def test_argmin_selects_minimum(self):
        """Value at the returned index equals the actual min along axis."""
        np.random.seed(11)
        data = np.random.randn(3, 4, 5).astype(np.float64)
        axis = 2
        actual, _, _ = _run_argmin(data, axis=axis, keepdims=0, tag="selects_min")
        ...
        true_min = np.min(data, axis=axis)
        np.testing.assert_allclose(gathered, true_min)

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_invariant_to_offset(self):
        """argmin(X + c) == argmin(X) for any constant c."""
        np.random.seed(13)
        data = np.random.randn(3, 8).astype(np.float64)
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="offset_base")
        shifted, _, _ = _run_argmin(
    ...
    def test_invariant_to_positive_scale(self):
        """argmin(c * X) == argmin(X) for c > 0."""
        np.random.seed(17)
        data = np.random.randn(4, 6).astype(np.float64)
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="scale_base")
        scaled, _, _ = _run_argmin(data * 7.5, axis=1, keepdims=0, tag="scale_pos")
    ...
    def test_flips_with_negative_scale(self):
        ...
        am_neg, _, _ = _run_argmin(-data, axis=1, keepdims=0, tag="neg_scale")
        argmax_ref = np.argmax(data, axis=1).astype(np.int64)
        np.testing.assert_array_equal(am_neg, argmax_ref)

    @pytest.mark.unit
    @pytest.mark.opunit
    def test_unique_min_deterministic(self):
        """With all unique values, argmin is deterministic regardless of select_last_index."""
        np.random.seed(23)
        data = np.random.randn(3, 10).astype(np.float64)
        first, _, _ = _run_argmin(
            data, axis=1, keepdims=0, select_last_index=0, tag="uniq_first"
        )
        last, _, _ = _run_argmin(
    ...
    def test_batch_independence(self):
        """ArgMin on each batch element matches individual computation."""
        np.random.seed(29)
        data = np.random.randn(4, 8).astype(np.float64)
        actual, _, _ = _run_argmin(data, axis=1, keepdims=0, tag="batch_ind")
        for i in range(4):
            row_am, _, _ = _run_argmin(
...
# 6. Memory validation
# ===================================================================
def calculate_argmin_memory_stats(
    op, device, input_shape, axis, keepdims, precision="fp16"
):
    """
    Calculate memory performance metrics for a single argmin operation.

    Args:
        op: SimOp representing the argmin operation
...
def test_argmin_memory_validation(capsys, request):
    """
    Test memory validation for argmin operation.
    Validates instructions executed and data moved for various scenarios.

    Run with: pytest tests/test_ops/test_argmin.py::test_argmin_memory_validation -v
    ...
    test_cases = [
        {
            ...
            "axis": 0,
            "keepdims": 1,
            "description": "Find min across rows",
        },
        {
            "name": "2D Col ArgMin",
            "shape": [32, 64],
            "axis": 1,
            "keepdims": 0,
            "description": "Find min across columns",
        },
        {
            "name": "3D Channel ArgMin",
            "shape": [2, 64, 16],
            "axis": 1,
            "keepdims": 1,
            "description": "Find min across channels",
        },
        {
            "name": "4D Batch ArgMin",
            "shape": [8, 16, 16, 32],
            "axis": 3,
            "keepdims": 0,
            "description": "Find min in last dimension",
        },
    ]
    ...
    for test_case in test_cases:
        ...
        op_info = {
            "name": f'argmin_mem_{test_name.replace(" ", "_")}',
            "optype": "ArgMin",
            "inList": [data_t.name],
            "outList": [out_t.name],
            "attrs": {
        ...
        mem_stats = calculate_argmin_memory_stats(
        ...
        if mem_stats:
            ...
            # ArgMin output is always int64 (8 bytes per element)
    ...
    try:
        ...
        if terminalreporter:
            terminalreporter.write_sep(
                "=", "ARGMIN MEMORY VALIDATION RESULTS", bold=True, green=True
            )
