...
class TestMatMul_INT8:
    ...
    def test_factory_integration(self):
        """Test that MatMul_INT8 is registered in the op mapping"""
        assert get_opdesc_registry().has_shape_inference_function('MatMul_INT8')
    ...
    def test_invalid_input_count(self):
        ...
        with pytest.raises(AssertionError, match=r"#inputs for SimOp.* should be in range"):
    ...
    def test_invalid_output_count(self):
        ...
        with pytest.raises(AssertionError, match=r"#outputs for SimOp.* should be in range"):
            op.get_perf_counts(inT, outT)
    ...
    def test_invalid_dtype(self):
        ...
        with pytest.raises(AssertionError, match="MatMul_INT8 expects integer inputs"):
    ...
    def test_incompatible_dimensions(self):
        ...
        with pytest.raises(ValueError, match="Matmul incompatible"):
            op.get_perf_counts([A, B], [C])
    ...
    def test_attributes_not_allowed(self):
        ...
        with pytest.raises(AssertionError, match="MatMul_INT8 should have no attributes"):
            ...
            SimOp(op_info).get_perf_counts([make_tensor('A'), make_tensor('B')], [make_tensor('C')])
