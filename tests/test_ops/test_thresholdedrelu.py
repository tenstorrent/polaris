...
class TestThresholdedRelu:
    ...
    def test_thresholdedrelu_invalid_inputs(self):
        ...
        with pytest.raises(AssertionError, match="should be in"):
            op.get_perf_counts([make_tensor('input1'), make_tensor('input2')], [make_tensor('output')])
        ...
        op = SimOp(op_info)
        with pytest.raises(AssertionError, match="should be in"):
            op.get_perf_counts([make_tensor('input')], [make_tensor('output1'), make_tensor('output2')])
    ...
    def test_thresholdedrelu_invalid_tensor_shape(self):
        ...
        # ThresholdedRelu allows scalar input in the desc-based implementation
        assert perf_stats is not None
    ...
    def test_thresholdedrelu_thresholding_behavior(self):
        ...
        for alpha in alpha_values:
            ...
            # All thresholded operations should have comparison and movement
            total_elems = np.prod(input_shape)
            assert perf_stats['instrs']['cmp'] == total_elems
            assert perf_stats['instrs']['mov'] >= total_elems
