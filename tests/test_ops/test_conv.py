...
def test_conv():
    ...
    for tno, trec in enumerate(test_cases):
        ...
        attrs = {}
        if "d" in trec:
            attrs["dilations"] = trec["d"]  # type: ignore
        if "g" in trec:
            attrs["group"] = trec["g"]  # type: ignore
        if "p" in trec:
            attrs["pads"] = trec["p"]  # type: ignore
        if "s" in trec:
            attrs["strides"] = trec["s"]  # type: ignore
        if "a" in trec:
            attrs["auto_pad"] = trec["a"]  # type: ignore
        if "k" in trec:
            attrs["kernel_shape"] = trec["k"]  # type: ignore
        op_info = {
            "name": op_name,
            "optype": "Conv",
