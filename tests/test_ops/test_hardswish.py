...
class TestHardSwish:
    ...
    def test_factory_integration(self):
        """Basic sanity test for HardSwish creation via SimOp."""
        op_info = {
            'name': 'test_hardswish_factory',
            'optype': 'HardSwish',
            'inList': ['input'],
            'outList': ['output'],
            'attrs': {},
    ...
    def test_basic_hardswish_functionality(self):
        ...
        assert perf_stats['instrs'].get('add', 0) > 0  # Should have addition operations
        assert perf_stats['instrs'].get('cmp', 0) > 0  # Should have comparison operations
        assert perf_stats['instrs'].get('mul', 0) > 0  # Should have multiplication operations
        # Division is modeled for ONNXHardSwishOp; desc-based path may omit it
        # so we only require that, if present, it is non-zero.
        if 'div' in perf_stats['instrs']:
    ...
    def test_hardswish_large_tensor(self):
        ...
        # Verify instruction counts scale correctly (at least linear in elements)
        assert perf_stats['instrs'].get('add', 0) >= total_elements
        assert perf_stats['instrs'].get('cmp', 0) >= 2 * total_elements  # Two comparisons per element
        assert perf_stats['instrs'].get('mul', 0) >= total_elements
        if 'div' in perf_stats['instrs']:
    ...
    def test_hardswish_invalid_inputs(self):
        ...
        with pytest.raises(AssertionError, match="should be in"):
            SimOp(op_info).get_perf_counts([], [make_tensor('output')])
        ...
        with pytest.raises(AssertionError, match="should be in"):
            SimOp(op_info).get_perf_counts([make_tensor('input')], [make_tensor('output1'), make_tensor('output2')])
    ...
    def test_hardswish_invalid_tensor_shape(self):
        ...
        # Legacy HardSwish required 1D+, but new desc implementation handles scalars gracefully (elementwise).
        # We allow this to pass without raising AssertionError.
    ...
    def test_hardswish_hardware_friendly_properties(self):
        ...
        # Should have more comparisons than expensive transcendental ops (none expected here)
        assert perf_stats['instrs'].get('cmp', 0) > 0
        assert perf_stats['instrs'].get('add', 0) > 0
        assert perf_stats['instrs'].get('mul', 0) > 0

        # Verify instruction balance - HardSwish should be more ALU-heavy than FPU-heavy
        total_alu_ops = (
            perf_stats['instrs'].get('add', 0)
            + perf_stats['instrs'].get('cmp', 0)
            + perf_stats['instrs'].get('mul', 0)
        )
        total_fpu_ops = (
            perf_stats['instrs'].get('exp', 0)
            + perf_stats['instrs'].get('log', 0)
            + perf_stats['instrs'].get('div', 0)
        )
