...
class TestQAttention:
    ...
    def test_factory_integration(self):
        """Test that QAttention is registered in the op mapping"""
        from ttsim.ops.desc.registry import get_opdesc_registry
        assert get_opdesc_registry().has_shape_inference_function('QAttention')
    ...
    def test_qattention_custom_scale_factor(self):
        ...
        assert op.attrs.get('scale') == scale
    ...
    def test_qattention_unidirectional(self):
        ...
        assert perf_stats is not None
        assert op.attrs.get('unidirectional') == True
    ...
    def test_qattention_dequantized_attention_weights(self):
        ...
        assert op.attrs.get('attention_quantized') == False
    ...
    def test_invalid_quantization_scheme(self):
        ...
        with pytest.raises(ValueError, match="QAttention unsupported quantization_scheme"):
            op = SimOp(op_info)
            # Need to call get_perf_counts to trigger shape inference which validates scheme
            # Provide minimal dummy tensors
            t_dummy = [make_tensor(f't{i}') for i in range(9)]
            for t in t_dummy: t.dtype=np.dtype('float32') # Dtype doesn't matter for this check usually
            # But x needs check_shape
            t_dummy[0] = F._from_shape('x', [1,1,1], np_dtype='int8')
            t_dummy[3] = F._from_shape('w', [1,1], np_dtype='int8') # 1 hidden
            t_dummy[6] = F._from_shape('bias', [1], np_dtype='float32')
            op.get_perf_counts(t_dummy, [make_tensor('out')])
    ...
    def test_invalid_num_heads(self):
        ...
        with pytest.raises(ValueError, match="QAttention num_heads must be a positive integer"):
            ...
            t_dummy = [make_tensor(f't{i}') for i in range(9)]
            t_dummy[0] = F._from_shape('x', [1,1,1], np_dtype='int8')
            t_dummy[3] = F._from_shape('w', [1,1], np_dtype='int8')
            t_dummy[6] = F._from_shape('bias', [1], np_dtype='float32')
            op.get_perf_counts(t_dummy, [make_tensor('out')])
