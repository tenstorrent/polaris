...
def create_group_query_attention_ext_test_tensors(batch_size=2, seq_len_q=8, seq_len_k=6, num_heads=8,
    ...
    kv_hidden_size = kv_num_heads * head_dim
    ...
    # Query uses q_hidden_size, Key/Value use kv_hidden_size for GroupQueryAttention
    query = F._from_shape('query', [batch_size, seq_len_q, q_hidden_size], np_dtype=np_dtype)
    key = F._from_shape('key', [batch_size, seq_len_k, kv_hidden_size], np_dtype=np_dtype)
    value = F._from_shape('value', [batch_size, seq_len_k, kv_hidden_size], np_dtype=np_dtype)
    ...
    if use_past:
        past_seq_len = 4
        # Past key/value should have the same hidden size as current K/V tensors
        past_key = F._from_shape('past_key', [batch_size, past_seq_len, kv_hidden_size], np_dtype=np_dtype)
        past_value = F._from_shape('past_value', [batch_size, past_seq_len, kv_hidden_size], np_dtype=np_dtype)
    ...
    output_hidden_size = q_hidden_size  # Output has same size as query hidden size
    ...
    # k_head_size is derived from kv_hidden_size / kv_num_heads = head_dim
    kv_head_dim = kv_hidden_size // kv_num_heads  # Should be equal to head_dim
...
class TestGroupQueryAttentionExt:
    ...
    def test_factory_integration(self):
        """Test that GroupQueryAttentionExt is registered in the op mapping"""
        assert get_opdesc_registry().has_shape_inference_function('GroupQueryAttentionExt')
    ...
    def test_basic_extended_attention(self):
        ...
        # Present key/value should use head_dim, not inflated head_dim
        # Shape: [batch_size, kv_num_heads, seq_len_k, head_dim]
    ...
    def test_invalid_attention_type(self):
        ...
        with pytest.raises(ValueError, match="GroupQueryAttentionExt unsupported attention_type"):
            inT, outT, _ = create_group_query_attention_ext_test_tensors()
