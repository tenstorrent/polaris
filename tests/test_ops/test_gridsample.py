...
def test_gridsample_memory_validation(capsys, request):
    ...
    for test_case in test_cases:
        ...
        if mode == "nearest":
            ...
            logger.debug("  ✓ Low arithmetic intensity (memory-bound nearest mode)")
        else:
            # Bilinear has higher AI due to interpolation
            logger.debug("  ✓ Arithmetic intensity reflects bilinear interpolation")
        ...
        if mode == "nearest" and output_elems > 5000:
            ...
            logger.debug("  ✓ Memory-bound as expected (large nearest gridsample)")
