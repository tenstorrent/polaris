...
class TestEinsum:
    ...
    def test_factory_integration(self):
        """Test that Einsum is registered in the op mapping"""
        assert get_opdesc_registry().has_shape_inference_function('Einsum')
    ...
    def test_invalid_equation_format(self):
        ...
        inT, outT, _ = create_einsum_test_tensors([[4, 6]], 'ij->ij')
    ...
    def test_invalid_characters_in_equation(self):
        ...
        inT, outT, _ = create_einsum_test_tensors([[4, 6], [6, 8]], 'ij,jk->ik')
    ...
    def test_output_label_not_in_inputs(self):
        ...
        with pytest.raises(ValueError, match="Output labels .* contain labels not present in inputs"):
            SimOp(op_info).get_perf_counts([data], [F._from_shape('output', [4, 6], np_dtype=np.dtype('float32'))])
