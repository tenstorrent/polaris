...
"""
Unit tests for element-wise Sum operator.

Tests both shape inference (via SimOp.get_perf_counts) and numerical data
computation (via compute_elementwise_sum) through the full operator pipeline
...
from ttsim.ops.desc.data_compute import compute_elementwise_sum
...
# ============================================================================
# Test cases for 2-input element-wise Sum
# ============================================================================
...
two_input_test_cases = [
    ...
    ("Identity: A + zeros",            [2, 3],       [2, 3],       'zeros'),
...
# ============================================================================
# TEST: Element-wise Sum (2 inputs)
# ============================================================================

@pytest.mark.unit
@pytest.mark.opunit
def test_elementwise_sum_2_inputs():
    """Test element-wise Sum with 2 inputs: shape and numerical validation."""
    test_name = 'test_elementwise_sum_2'
    ...
    for tno, (tmsg, shape_A, shape_B, data_type) in enumerate(two_input_test_cases):
        ...
        # Reference: numpy addition (element-wise sum of 2 inputs)
        ref_output = (data_A + data_B).astype(np.float32)
        ...
        try:
            computed_output = compute_elementwise_sum(i_tensors, op_obj)
        ...
        # 4. Perf stats validation: should use 'add' instructions
        perf_ok = True
...
# ============================================================================
# TEST: Element-wise Sum (3+ inputs)
# ============================================================================

@pytest.mark.unit
@pytest.mark.opunit
def test_elementwise_sum_3_inputs():
    """Test element-wise Sum with 3 inputs."""
    ...
    ref_output = data_A + data_B + data_C
    ...
    op_obj = SimOp(op_info)
    for x in i_tensors: x.op_in  = ['sum_3_inputs']
    for x in o_tensors: x.op_out = ['sum_3_inputs']

    op_obj.get_perf_counts(i_tensors, o_tensors)

    assert o_tensors[0].shape == list(ref_output.shape), \
        f"Shape mismatch: {o_tensors[0].shape} vs {list(ref_output.shape)}"
    assert o_tensors[0].data is not None, "No data propagated for 3-input Sum"
    ...
    # Perf stats should have n_inputs-1 = 2 adds per element
    instrs = op_obj.perf_stats.get('instrs', {})
    expected_adds = o_tensors[0].nelems() * 2  # 3 inputs → 2 adds per output element
...
@pytest.mark.unit
@pytest.mark.opunit
def test_elementwise_sum_4_inputs():
    """Test element-wise Sum with 4 inputs."""
    shape = [2, 3]
    ...
    op_obj = SimOp(op_info)
    for x in i_tensors: x.op_in  = ['sum_4_inputs']
    for x in o_tensors: x.op_out = ['sum_4_inputs']
...
# ============================================================================
# TEST: Element-wise Sum with broadcasting (3 inputs)
# ============================================================================

@pytest.mark.unit
@pytest.mark.opunit
def test_elementwise_sum_3_inputs_broadcast():
    """Test element-wise Sum with 3 inputs requiring broadcasting."""
    ...
    op_obj = SimOp(op_info)
    for x in i_tensors: x.op_in  = ['sum_3_bcast']
    for x in o_tensors: x.op_out = ['sum_3_bcast']
...
@pytest.mark.unit
@pytest.mark.opunit
def test_shape_only_sum():
    """Test that Sum with shape-only inputs (no data) still infers shape."""
    ...
    op_obj = SimOp(op_info)
    for x in i_tensors: x.op_in  = ['shape_only_sum']
    for x in o_tensors: x.op_out = ['shape_only_sum']
...
@pytest.mark.unit
@pytest.mark.opunit
def test_mixed_data_shape_sum():
    """Test Sum where one input has data and one is shape-only → no data output."""
    ...
    op_obj = SimOp(op_info)
    for x in i_tensors: x.op_in  = ['mixed_sum']
    for x in o_tensors: x.op_out = ['mixed_sum']
...
# ============================================================================
# TEST: Residual addition pattern (common in ResNets / transformers)
# ============================================================================

@pytest.mark.unit
@pytest.mark.opunit
def test_residual_pattern():
    """Test the residual addition pattern: output = x + residual.

    This is the pattern used in ResNet skip connections and transformer
    residual streams.
    """
    x = np.random.randn(2, 256, 8, 8).astype(np.float32)
    residual = np.random.randn(2, 256, 8, 8).astype(np.float32)
    ref_output = x + residual

    i_tensors = [
        F._from_data('x', x),
        F._from_data('residual', residual),
    ]
    o_tensors = [make_tensor('Y')]
    ...
    op_obj = SimOp(op_info)
    for t in i_tensors: t.op_in  = ['residual_sum']
    for t in o_tensors: t.op_out = ['residual_sum']
...
if __name__ == '__main__':
    np.random.seed(42)
    test_elementwise_sum_2_inputs()
    test_elementwise_sum_3_inputs()
    test_elementwise_sum_4_inputs()
    test_elementwise_sum_3_inputs_broadcast()
    test_shape_only_sum()
...
def calculate_sum_memory_stats(input_shapes, n_inputs, precision='fp32'):
    """
    Calculate expected memory stats for an element-wise Sum operation.
    ...
    # Instructions: (n_inputs - 1) adds per output element (no div for Sum)
...
class TestSumMemoryValidation:
    """Validate memory estimation and instruction counts for Sum operation."""

    @pytest.mark.unit
    @pytest.mark.opunit
    @pytest.mark.performance
    def test_sum_memory_validation(self, capsys, request):
        """
        Validate memory usage and instruction counts for element-wise Sum.

        This test validates two primary metrics:
        1. Instructions Executed: (N-1) 'add' per output element
        2. Data Moved: Tracks input/output bytes and validates memory traffic

        Sum computes element-wise sum of N tensors: Y = T1 + T2 + ... + TN
        ...
        for input_shapes, description in memory_validation_cases:
            ...
            result = compute_elementwise_sum(i_tensors, op)
            np.testing.assert_allclose(
                result, ref_output, rtol=1e-5, atol=1e-6,
                err_msg=f"[{description}] compute_elementwise_sum validation failed",
            )

            # Calculate expected stats
            expected_stats = calculate_sum_memory_stats(input_shapes, n_inputs, 'fp32')
        ...
        summary_lines = [
            ...
            "",
            "Key Findings:",
            "  * Instructions: (N-1) 'add' per output element (no div) [PASS]",
            "  * More inputs -> higher arithmetic intensity",
        ...
        for result in all_results:
            summary_lines.append(
                "  [PASS] {:<26s} | {:>7,} add | {:>7.1f} KB | {:.3f} ops/byte | N={:d}".format(
                    result['test_name'],
