...
def test_attention_invalid_shapes():
    ...
    try:
        ...
        _ = op_obj.get_perf_counts([Q, K, V], [output])
        assert False, "Should have raised ValueError for mismatched K/V kv_heads"
...
def test_attention_scale_attribute():
    ...
    assert op_obj.attrs['scale'] == custom_scale
    print(f"✓ Custom scale attention test passed")


def test_attention_descriptor_registered():
    """Test that Attention operation is registered in mapping"""
    print("\n=== Testing Attention Descriptor Registration (ONNX) ===")
    assert get_opdesc_registry().has_shape_inference_function('Attention')


if __name__ == '__main__':
    # Run all tests manually
    test_attention_descriptor_registered()
