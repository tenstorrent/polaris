...
def test_multihead_attention_with_past_states():
    ...
    expected_present_key_shape = [batch_size, num_heads, seq_len + seq_len//2, hidden_size//num_heads]  # [B, H, S_total, D]
    expected_present_value_shape = [batch_size, num_heads, seq_len + seq_len//2, hidden_size//num_heads]
...
def test_multihead_attention_invalid_inputs():
    ...
    try:
        ...
        op_obj = SimOp(op_info)
        # Check happens in get_perf_counts or shape inference
        # Need actual tensors to trigger shape inference
        ...
        output = make_tensor('output')
        op_obj.get_perf_counts([query, key, value], [output])
    ...
    try:
        ...
        assert False, "Should have raised ValueError for head count mismatch"
    except ValueError as e:
        print(f"✓ Correctly caught head count mismatch: {e}")
    ...
    try:
        ...
        perf_stats = op_obj.get_perf_counts([query, key, value], [output])
        assert False, "Should have raised ValueError for mismatched hidden sizes"
    except ValueError as e:
...
def test_multihead_attention_unidirectional():
    ...
    assert op_obj.attrs.get('unidirectional') == True
...
def test_multihead_attention_factory():
    """Test that MultiHeadAttention operation is registered in the op mapping"""
    print("\n=== Testing MultiHeadAttention Factory Creation ===")
    from ttsim.ops.desc.registry import get_opdesc_registry
    assert get_opdesc_registry().has_shape_inference_function('MultiHeadAttention')
