...
def test_gather_nd_with_batch_dims():
    ...
    print(f"  Data shape: {input_tensors[0].shape}")
    print(f"  Indices shape: {input_tensors[1].shape}")
    print(f"  Batch dims: {op_obj.attrs['batch_dims']}")
    print(f"  Output shape: {output_tensors[0].shape}")
...
def test_gather_nd_invalid_inputs():
    ...
    try:
        ...
        op_info = {
            ...
            'inList': ['data', 'indices'],
            'outList': ['output'],
            'attrs': {'batch_dims': 100},  # Invalid large value
        }

        op_obj = SimOp(op_info)
        op_obj.get_perf_counts(input_tensors, output_tensors)
    ...
    try:
        ...
        op_obj = SimOp(op_info)
        perf_stats = op_obj.get_perf_counts(input_tensors, output_tensors)
        assert False, "Should have raised ValueError for index depth too large"
    except ValueError as e:
...
def test_gather_nd_factory():
    """Test that GatherND operation is registered in the op mapping"""
    print("\n=== Testing GatherND Factory Creation ===")
    from ttsim.ops.desc.registry import get_opdesc_registry
    assert get_opdesc_registry().has_shape_inference_function('GatherND')
