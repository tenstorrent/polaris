...
from ttsim.utils.common import (
    get_ttsim_functional_instance,
    get_ttnn_functional_instance,
    print_csv,
    str_to_bool,
    setup_logger,
)
...
class RangeArgument:
    def __init__(self, name, arg, range_type='add'):
        ...
        if arg is not None:
            ...
            assert range_type in ['add', 'mul'], f'range-arg: {name}, range_type({range_type}) can only be (add|mul)'
            x = start
            while x <= end:
                self.vals.append(x)
...
def get_wlgraph(TBL, wlg, wln, wli, gcfg, wpath, enable_memalloc):
    ...
    if num_xrows == 1 and xrows[0][3] is None:
        ...
        assert (wlg, wln, wli, wlb) in TBL, f'Workload= {wlg}.{wln}.{wli}.b{wlb} missing in workloads graph table!!'
    ...
    if TBL[(wlg, wln, wli, wlb)] is None:
        if wlg == 'TTSIM':
            ...
            DEBUG(
                '>>ttsim-wl analytical parameter count {}.{}.{}.b{}= {:,d}',
                wlg,
                wln,
                wli,
                wlb,
            ...
            try:
                ...
                DEBUG('>>ttnn-wl analytical parameter count {}.{}.{}.b{}= {:,d}', wlg, wln, wli, wlb, 0)
            finally:
...
def do_instr_profile(_WLG, _ODIR):
    ...
    for _wlx, (wlobj, wlgraph) in _WLG.items():
        ...
        for i, x in enumerate(wlgraph.get_ordered_nodes()):
            ...
            for k, v in fwd_op.perf_stats['instrs'].items():
                if k not in ITBL[_wlx]:
                    ITBL[_wlx][k] = 0  # noqa: E701
                ITBL[_wlx][k] += v
    ...
    for wlg, wln, wli, wlb in ITBL:
        ...
        for instr, count in instr_tbl.items():
            profile_data.append(
                {'group': wlg, 'workload': wln, 'instance': wli, 'batchsize': wlb, 'instruction': instr, 'count': count}
            )
...
def setup_cmdline_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        '--instr_profile', action='store_true', default=False, help='Collect Instruction Profile for Workloads'
    )
    parser.add_argument(
        '--dump_ttsim_onnx', action='store_true', default=False, help='Dump ONNX graph for TTSIM Workload'
    )
    ...
    parser.add_argument(
        ...
        default=None,
        choices=data_types,
        help='Activation Data Type to use for the projection',
    )

    parser.add_argument(
        '--filterwlg', default=None, help='use only workload-groups specified in filterwlg (comma sep list)'
    )
    parser.add_argument('--filterwl', default=None, help='use only workloads specified in filterwl (comma sep list)')
    parser.add_argument(
        '--filterwli', default=None, help='use only workload instances specified in filterwli (comma sep list)'
    )
    parser.add_argument(
        '--filterarch', default=None, help='use only architectures specified in filterarch (comma sep list)'
    )
    parser.add_argument(
        '--frequency',
        nargs=3,
        ...
        type=int,
        action='append',
        help='frequency (in MHz) range specification (arith-seq)',
    )
    parser.add_argument(
        '--batchsize',
        nargs=3,
        ...
        type=int,
        action='append',
        help='batchsize range specification (geom-seq)',
    )
    parser.add_argument('--outputformat', choices=['none', 'yaml', 'json', 'pickle'], default='json', type=str.lower)
    parser.add_argument(
        '--dump_stats_csv', dest='dump_stats_csv', action='store_true', default=False, help='Dump stats in CSV format'
    )
    parser.add_argument(
        '--opstats_startnum',
        dest='opstats_startnum',
        type=int,
        default=0,
        help='Start number for operator stats.  Default=0',
    )
...
def execute_wl_on_dev(
    ...
    for exp_no, (exp_wl, exp_dev) in enumerate(ALL_EXPS):
        ...
        try:
            wlobj, wlgraph = get_wlgraph(_WLG, wlgroup, wlname, wlins_name, wlcfg, wlpath, _enable_memalloc)
        ...
        try:
            ...
            stats_info = {
                ...
                'odir': _odir,
                'opstats_startnum': opstats_startnum,
            }
            cur_device = Device(dev_obj)
            cur_device.execute_graph(wlgraph, wlmapspec, opstats_startnum)
...
def polaris(args: argparse.Namespace | runcfgmodel.PolarisRunConfig) -> int:
    ...
    if args.dryrun:
        ...
        num_failures, summary_stats = execute_wl_on_dev(
            ...
            outputformat,
            args.dump_stats_csv,
            args.opstats_startnum,
        )
        summary_stats = sorted(
            summary_stats, key=lambda x: (x['wlname'], x['devname'], x['freq_Mhz'], x['wlinstance'], x['bs'])
        )
