...
ENABLE_TILE_SCALEFACTOR_DEBUG = False
TILE_SCALE_FACTOR            = 1
...
class triscFunc:
    ...
    def __execsw__(self, ins, cycle):
        ...
        if(regType == '') and (cfgRegType == ''):  # Not MMR
            ...
            print(f"__execsw(tileCounters): {cfgRegType}, {cfgRegIndex}, {hex(self.triscRegs.__readReg__(ins.getSrcInt()[0]) + ins.getImm()[0])}")
            ...
            match bufferIntraRegIndex:
                ...
                case 2: # TILES_AVAILABLE
                    ...
                    spaceAvail = self.ttSplRegs.__readReg__(bufferRegIndex, cfgRegType,'space_available')
                    # assert spaceAvail >= self.triscRegs.__readReg__(ins.getSrcInt()[1]),f"tiles_available {hex(self.triscRegs.__readReg__(ins.getSrcInt()[1]))} cannot be greater than space_available {hex(spaceAvail)}"
                    if spaceAvail >= self.triscRegs.__readReg__(ins.getSrcInt()[1]):
                        print(f"WARNING: tiles_available {hex(self.triscRegs.__readReg__(ins.getSrcInt()[1]))} cannot be greater than space_available {hex(spaceAvail)}")
                ...
                case 3: # SPACE_AVAILABLE
                    print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{self.threadId} Pipe:[RISC] Instruction:{ins.getOp()} tile_counters[{hex(bufferRegIndex)}] Writing to (TENSIX):{hex(self.triscRegs.__readReg__(ins.getSrcInt()[0]) + ins.getImm()[0])} tile_counters[{hex(bufferRegIndex)}][space_available]={hex(self.triscRegs.__readReg__(ins.getSrcInt()[1]))}")
    ...
    def __execaddi__(self, ins):
        ...
        if (ENABLE_TILE_SCALEFACTOR_DEBUG and ins.getRelAddr() == 0xe370 and ins.getThread() == 2) or (ins.getRelAddr() == 0x6494 and ins.getThread() == 0) or (ins.getRelAddr() == 0xa2c4 and ins.getThread() ==1):
            print(f"Debug ADDI at address {hex(ins.getRelAddr())} for thread {ins.getThread()}")
            if(self.debug & 0x10):
                print(f"\t{ins.getOp()}: {hex(self.triscRegs.__readReg__(ins.getSrcInt()[0]))} + {hex(ins.getImm()[0] * TILE_SCALE_FACTOR)} = {hex(self.triscRegs.__readReg__(ins.getSrcInt()[0]) + (ins.getImm()[0] * TILE_SCALE_FACTOR))}")
            self.triscRegs.__writeReg__(ins.getDstInt()[0], self.triscRegs.__readReg__(ins.getSrcInt()[0],'riscgpr') + (ins.getImm()[0] * TILE_SCALE_FACTOR))
        else:
            if(self.debug & 0x10):
    ...
    def __execlw__(self, ins, cycle):
        ...
        if(regType == '') and (cfgRegType == ''):  # Not MMR
            ...
            match bufferIntraRegIndex:
                case 1: #RESET
                    print(f"WARNING: Addr:{hex(ins.getRelAddr())} Thread{self.threadId} Don't know what to do with read RESET tile_counters[{hex(bufferIntraRegIndex)}] register")
                    self.triscRegs.__writeReg__(ins.getDstInt()[0], self.ttSplRegs.__readReg__(bufferRegIndex, cfgRegType,'reset'))
                case 2: # TILES_AVAILABLE
                    print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} Thread{self.threadId} Pipe:[RISC] Instruction:{ins.getOp()} tile_counters[{hex(bufferRegIndex)}] Reading from (TENSIX):{hex(self.triscRegs.__readReg__(ins.getSrcInt()[0]) + ins.getImm()[0])} tile_counters[{hex(bufferRegIndex)}][tiles_available]={hex(self.ttSplRegs.__readReg__(bufferRegIndex, cfgRegType,'tiles_available'))}")
