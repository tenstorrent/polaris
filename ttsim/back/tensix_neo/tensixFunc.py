...
class ttSplRegs:
    ...
    def __writeReg__(self,r, val, type='riscgpr', subr = None):
        ...
        if subr == None:
            ...
            print("{0}[{1}][{2}] = {3}".format(type, r, subr, val))
    ...
    def __readReg__(self,r, type='riscgpr', subr=None):
        ...
        if (type == 'riscgpr' and (r in self.regTempsriscgpr)): # Temporary Registers - RISCV
            assert subr == None, f"Sub-register read not supported for register type: {type}"
...
class tensixFunc:
    ...
    def __execpushtiles__(self,ins, cycle):
        ...
        print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Writing to (TENSIX) incrVal={hex(incrVal)} tile_counters[{hex(buffer)}][tiles_available]={hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "tiles_available") + incrVal)})")
        ...
        print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Writing to (TENSIX) decrVal={hex(incrVal)} tile_counters[{hex(buffer)}][space_available]={hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "space_available") - incrVal)})")
    ...
    def __execpoptiles__(self,ins, cycle):
        ...
        print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Writing to (TENSIX) decrVal={hex(incrVal)} tile_counters[{hex(buffer)}][tiles_available]={hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "tiles_available") - incrVal)})")
        self.tensixSplRegs.__writeReg__(buffer, self.tensixSplRegs.__readReg__(buffer, "tileCounters", "tiles_available") - incrVal, "tileCounters", "tiles_available")
        print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Writing to (TENSIX) incrVal={hex(incrVal)} tile_counters[{hex(buffer)}][space_available]={hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "space_available") + incrVal)})")
        self.tensixSplRegs.__writeReg__(buffer, self.tensixSplRegs.__readReg__(buffer, "tileCounters", "space_available") + incrVal, "tileCounters", "space_available")
    ...
    def __execwaittiles__(self,ins, cycle):
        ...
        if self.tensixSplRegs.__readReg__(buffer, "tileCounters", "tiles_available") < targetTiles:
            print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Pipe[{ins.getExPipe()}] Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Stalling on WAIT_TILES instruction until tiles_available:{hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "tiles_available"))} < {hex(targetTiles)}")
            ins.setExPipe('SYNC')
            pipeLst2  = self.pipeGrps['SYNC']
        else:
            print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Pipe[{ins.getExPipe()}] Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Resuming from WAIT_TILES instruction as tiles_available:{hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "tiles_available"))}  >= {hex(targetTiles)}")
    ...
    def __execwaitfree__(self,ins, cycle):
        ...
        if self.tensixSplRegs.__readReg__(buffer, "tileCounters", "space_available") < targetTiles:
            print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Pipe[{ins.getExPipe()}] Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Stalling on WAIT_FREE instruction until space_available:{hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "space_available"))} < {hex(targetTiles)}")
            ins.setExPipe('SYNC')
            pipeLst2  = self.pipeGrps['SYNC']
        else:
            print(f"Cycle:{cycle} Addr:{hex(ins.getRelAddr())} TCore{self.coreId} Thread{ins.getThread()} Pipe[{ins.getExPipe()}] Instruction:{ins.getOp()} tile_counters[{hex(buffer)}] Resuming from WAIT_FREE instruction as space_available:{hex(self.tensixSplRegs.__readReg__(buffer, "tileCounters", "space_available"))}  >= {hex(targetTiles)}")
