...
class MEM(Component):
    ...
    def __str__(self):
        return 'MEM ' + super().__str__() + f' size={self.size_nbytes:,d}B' + f' bw={self.bw_bytes_per_clk:,d}B/Clk'
...
class NOC(Component):
    ...
    def __str__(self):
        return 'NOC ' + super().__str__() + f' nrows={self.nrows}, ncols={self.ncols}'
...
class Device:
    ...
    def execute_graph(self, wlgraph, wlmapspec, opstats_startnum):
        ...
        for fusion_nodes in fusion_candidates:
            ...
            for i in range(1, pattern_len):
                ...
                fused_compute_cycles += math.ceil(
                    matched_op.compute_cycles * (1.0 + self.G_FUSE_OP_OVERLAP_COST_CONSTANT)
                )
    ...
    def get_exec_stats(self, wlgraph, bs, opstats_startnum):
        graph_ordered_nodes = wlgraph.get_ordered_nodes()
        ...
        tot_maxActBytes = max(
            x + y
            for x, y in zip(
                op_stat_iter('inActCount', repeat=False, use_precision=True),
                op_stat_iter('outActCount', repeat=False, use_precision=True),
            )
        )

        tot_comp_bound_cycles = sum(
            y for x, y in zip(op_stat_iter('rsrc_bnck'), op_stat_iter('ideal_cycles', repeat=True)) if x == 'COMP'
        )
        tot_mem_bound_cycles = sum(
            y for x, y in zip(op_stat_iter('rsrc_bnck'), op_stat_iter('ideal_cycles', repeat=True)) if x == 'MEM'
        )
