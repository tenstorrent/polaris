...
class HLMStats:
    def __init__(self, _dev, _wlgraph, _wlinfo, _sinfo):
        ...
        self.opstats_startnum = _sinfo['opstats_startnum']
    ...
    def dump_stats(self, dfreq):
        summary_dict = self.device.get_exec_stats(self.wlgraph, self.batchsize, self.opstats_startnum)

        graph_ordered_nodes = self.wlgraph.get_ordered_nodes()

        opstats_tbl = []
        for opnum, opname in enumerate(graph_ordered_nodes):
            if opnum < self.opstats_startnum:
                continue
