...
def _write_xlsx_report(
    ...
    *,
    ...
    if have_profiler:
        prof_cnt, prof_ms, prof_lut = _layer_totals(profiler_layers)  # type: ignore[arg-type]
    if have_polaris:
        pol_cnt, pol_ms, pol_lut = _layer_totals(polaris_layers)  # type: ignore[arg-type]
    ...
    for k in keys_sig:
        op, sig = k
        p_cnt, p_ms, _ = prof_by_sig.get(k, (0, 0.0, 0))
        s_cnt, s_ms, s_lut = pol_by_sig.get(k, (0, 0.0, 0))
        s_miss = s_cnt - s_lut
        only_pol = max(0, s_cnt - p_cnt)
        only_hw = max(0, p_cnt - s_cnt)
