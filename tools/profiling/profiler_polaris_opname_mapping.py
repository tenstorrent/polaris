...
``MatMul_0``-style names from :mod:`profiler_to_polaris_converter`.
...
* ``fingerprint`` (default): (``optype``, input shapes, output shapes) using the same optype and
  WZYX parsing rules as ``profiler_to_polaris_converter``. Stable for duplicate op signatures by
...
from profiler_to_polaris_converter import (  # noqa: E402
...
def profiler_op_signature(
    ...
    *,
    ...
    :func:`profiler_to_polaris_converter.parse_tensor_dimensions` for ``TilizeWithValPadding``
