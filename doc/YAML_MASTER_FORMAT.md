...
- **Producer behavior:** Non-whole **`CORE COUNT`** in Excel is coerced with **`int(round(...))`** for **`num_cores`** and a log **warning** (see spec).
...
Special cases (same strings as in spec): all-NaN/zero stats use **`_curve_zero_law_entry`**-style payloads (`a`/`b`/`r2`/`equation` as documented in **`doc/SPEC_tt_perf_mapper.md`**). The entry’s **`curve_family`** still reflects the duration-based choice; zero-law stats do not change **`curve_family`**.
