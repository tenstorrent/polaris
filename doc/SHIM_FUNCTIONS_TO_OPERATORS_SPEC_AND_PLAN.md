...
Use exact names Tilize, Untilize, TilizeWithValPadding, UntilizeWithUnpadding (matching the existing function names tilize_with_val_padding / untilize_with_unpadding)
...
- **New front-end APIs:** `ttsim/front/ttnn/ttnn_shim.py` only — `tilize_op`, `untilize_op`, `tilize_with_val_padding_op`, `untilize_with_unpadding_op`. Tracking-only (create SimOp, add to device); no execution, no `_tracker`.
...
- **untilize_with_unpadding_sinf:** 1 input. Output logical = op.attrs['output_shape']. perf_stats from shapes.
- **register_layout_ops():** register_ops('layout', _optbl) with Tilize, Untilize, TilizeWithValPadding, UntilizeWithUnpadding (1→1 each), shape_inf_func = the four sinf callables; table columns same as tensor.py/nn.py.
...
- **untilize_with_unpadding_op(input_tensor, output_shape, use_multicore=True, use_pack_untilize=True, element_size=2, ...):** attrs output_shape (list/tuple). SimOp optype 'UntilizeWithUnpadding'.
...
| UntilizeWithUnpadding  | 1→1   | untilize_with_unpadding_op(input_tensor, output_shape, ...) | output_shape, use_multicore, use_pack_untilize, element_size |
