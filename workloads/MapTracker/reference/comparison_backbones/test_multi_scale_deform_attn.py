...
def test_basic_single_level():
    ...
    assert np.allclose(
        output_pytorch_np, output_ttsim_np, rtol=1e-4, atol=1e-5
...
def test_batch_size_variations():
    ...
    for bs in [1, 2, 4]:
        ...
        print(
            f"  Max diff: {np.abs(output_pytorch_np - output_ttsim_np).max():.6e}, Mean diff: {np.abs(output_pytorch_np - output_ttsim_np).mean():.6e}"
        )
...
def test_full_module_normalizer_shape():
    ...
    print(
        f"  num_levels={num_levels}, num_points={num_points} (intentionally different)"
    )
    ...
    for linear_name in [
        ...
        linear.param.data = np.random.uniform(-limit, limit, linear.param.shape).astype(
            np.float32
        )
    ...
    query = F_ttsim._from_data(
        "query", np.random.randn(bs, num_query, embed_dims).astype(np.float32)
    )
    value = F_ttsim._from_data(
        "value", np.random.randn(bs, num_value, embed_dims).astype(np.float32)
    )
    reference_points = F_ttsim._from_data(
        "ref_pts", np.random.rand(bs, num_query, num_levels, 2).astype(np.float32)
    )
...
def test_full_module_identity_non_batch_first():
    ...
    query = F_ttsim._from_data(
        "query_nbf", np.random.randn(num_query, bs, embed_dims).astype(np.float32)
    )
    value = F_ttsim._from_data(
        "value_nbf", np.random.randn(num_value, bs, embed_dims).astype(np.float32)
    )
    # reference_points is always [bs, num_query, num_levels, 2]
    reference_points = F_ttsim._from_data(
        "ref_pts_nbf", np.random.rand(bs, num_query, num_levels, 2).astype(np.float32)
    )

    # Provide an explicit identity tensor to test residual path
    identity = F_ttsim._from_data(
        "identity_nbf", np.random.randn(num_query, bs, embed_dims).astype(np.float32)
    )
