...
def pytorch_get_reference_points(
    ...
    if dim == "3d":
        ...
        # change y-axis direction (matches actual PyTorch source)
        ys = torch.linspace(H - 0.5, 0.5, H, dtype=dtype, device=device).view(1, H, 1)
...
def test_reference_points_3d():
    ...
    try:
        ...
        if match:
            ...
            print("\n[X] 3D reference point generation test failed!")
            print("  PyTorch and TTSim outputs differ.")
            return False

    except Exception as e:
        print(f"\n[X] 3D reference point test failed: {e}")
        traceback.print_exc()
        return False
...
def test_reference_points_2d():
    ...
    try:
        ...
        if match:
            ...
            print("\n[X] 2D reference point generation test failed!")
            print("  PyTorch and TTSim outputs differ.")
            return False

    except Exception as e:
        print(f"\n[X] 2D reference point test failed: {e}")
        traceback.print_exc()
        return False
...
def test_point_sampling():
    ...
    try:
        ...
        if visible_ratio > 0:
            ...
            print(f"   [WARN] WARNING: No visible points!")
        ...
        print(f"\n[X] Point sampling test failed: {e}")
        traceback.print_exc()
        return False
...
def test_bevformer_layer_construction():
    ...
    try:
        ...
        print(f"   [OK] Parameter count calculated successfully")

        print("\n[OK] BEVFormerLayer construction test passed!")
        return True

    except Exception as e:
        print(f"\n[X] BEVFormerLayer construction test failed: {e}")
        traceback.print_exc()
        return False
...
def test_bevformer_encoder_construction():
    ...
    try:
        ...
        print(f"\n2. Encoder Structure:")
        print(f"   [OK] Encoder constructed successfully")
        ...
        # Note: The encoder's __call__ uses F.ConcatX to concatenate
        # shift_ref_2d (a SimTensor from F.Reshape+F.Add on the shift input)
        # with ref_2d when building hybrid_ref_2d. This keeps the shift
        # Reshape+Add ops graph-connected in the ONNX export.

        print("\n[OK] BEVFormerEncoder construction test passed!")
        return True

    except Exception as e:
        print(f"\n[X] BEVFormerEncoder construction test failed: {e}")
        traceback.print_exc()
        return False
...
def main():
    ...
    if passed_tests == total_tests:
        ...
        print("   - PyTorch vs TTSim: Reference point generation matches exactly")
        print("   - Shape validation: All operations produce correct shapes")
        print("   - Module construction: All components build successfully")
        print("   - No PyTorch/MMCV dependencies in TTSim implementation")
        print("\n[TODO] Note on Data Validation:")
        print("   - Realistic camera matrices: Based on nuScenes dataset calibration")
        print(
        ...
        print("\n[Production Usage]:")
        print("   To test with actual model and real data:")
        print("   1. Load pre-trained BEVFormer weights")
        print("   2. Get img_metas from dataset: img_metas = dataset[idx]['img_metas']")
        print("   3. Real img_metas contain actual camera calibration from sensors")
        print(
            "   4. Forward pass: output = encoder(bev_query, key, value, ..., img_metas)"
        )
        print("   5. Camera matrices come from sensor calibration files in dataset")
        return 0
    else:
