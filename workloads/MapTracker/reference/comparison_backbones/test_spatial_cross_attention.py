...
class MSIPM3D_PyTorch(nn.Module):
    """PyTorch reference implementation of MSIPM3D for validation.

    This is a simplified deformable attention module with:
    ...
    def __init__(
        self, embed_dims=256, num_heads=8, num_levels=4, num_points=8, batch_first=True
    ):
        super().__init__()
        self.embed_dims = embed_dims
        self.num_heads = num_heads
        ...
        # Initialize value_proj with xavier uniform
        nn.init.xavier_uniform_(self.value_proj.weight)
        nn.init.constant_(self.value_proj.bias, 0.0)

    def forward(
        self, query, value=None, reference_points=None, spatial_shapes=None, **kwargs
    ):
        """
        Args:
            query: [bs, num_query, embed_dims]
            value: [bs, num_value, embed_dims]
            reference_points: [bs, num_query, num_Z_anchors, 2]
            spatial_shapes: list of (H, W) tuples or tensor [num_levels, 2]
        """
        if value is None:
            value = query
        ...
        # Create sampling offsets from fixed buffer
        ...
        # Convert spatial_shapes to list if needed
...
class SpatialCrossAttention_PyTorch(nn.Module):
    ...
    def forward(
        ...
        queries = (
            ...
            .reshape(bs * num_cams, num_query, self.embed_dims)
        )
...
def copy_pytorch_weights_to_ttsim(pytorch_module, ttsim_module):
    ...
    weight_np = (
        pytorch_module.sampling_offsets.weight.detach().cpu().numpy()
    )  # No transpose needed
...
def copy_sca_weights_to_ttsim(pytorch_sca, ttsim_sca):
    ...
    copy_pytorch_weights_to_ttsim(
        pytorch_sca.deformable_attention, ttsim_sca.deformable_attention
    )
...
def test_initialization_utils():
    ...
    try:
        ...
        if list(output_pt.shape) == list(output_ttsim.shape):
            ...
            print(
                f"    [X] Shape mismatch: PyTorch {list(output_pt.shape)} vs TTSim {list(output_ttsim.shape)}"
            )
            all_passed = False
...
def test_sca_forward():
    ...
    try:
        ...
        if not shapes_ok:
            print(
                f"  [X] Shape mismatch: expected {expected_shape}, got {list(output_tt.shape)}"
            )
            return False
        ...
        if hasattr(output_tt, "data") and output_tt.data is not None:
            ...
            print(
                f"  TTSim:   mean={np.mean(output_tt_np):.6e}, std={np.std(output_tt_np):.6e}, "
...
def test_different_configurations():
    ...
    for i, (embed_dims, num_heads, num_levels, num_points, spatial_shapes) in enumerate(
        ...
        try:
            ...
            if list(output_pt.shape) == list(output_ttsim.shape):
                print(
                    f"  [OK] Shapes match! Parameter count: {msda3d.analytical_param_count():,}"
                )
            else:
                print(
                    f"  [X] Shape mismatch: PyTorch {list(output_pt.shape)} vs TTSim {list(output_ttsim.shape)}"
        ...
        try:
            ...
            if hasattr(output, "data") and output.data is not None:
                ...
                if batch_first:
                    ...
                    print(
                        f"    Max diff: {diff_array.max():.6e}, Mean diff: {diff_array.mean():.6e}"
                    )
                    # Numerical validation with np.allclose
                    if not np.allclose(
                        output_pt_np, output_ttsim_np, rtol=1e-4, atol=1e-5
                    ):
                        print(
                            f"    [Warning] Numerical outputs differ beyond tolerance"
                        )
                else:
                    ...
                    # Numerical validation with np.allclose
                    if not np.allclose(
                        output_pt_np, output_ttsim_np_t, rtol=1e-4, atol=1e-5
                    ):
                        print(
                            f"    [Warning] Numerical outputs differ beyond tolerance"
                        )
            ...
            if list(output.shape) == expected_shape and shapes_match:
                print(
                    f"  [OK] Output shape correct and matches PyTorch (accounting for batch_first)"
                )
            else:
                print(
                    f"  [X] Output shape incorrect: expected {expected_shape}, got {output.shape}"
                )
                all_passed = False
        except Exception as e:
            print(f"  [X] Test failed: {e}")
...
def test_with_masking():
    ...
    try:
        ...
        with torch.no_grad():
            output_pt_no_mask = model_pt(
                query_pt, value_pt, ref_points_pt, spatial_shapes
            )
        ...
        if (
            hasattr(output_ttsim_no_mask, "data")
            and output_ttsim_no_mask.data is not None
        ):
            ...
            print(
                f"    PyTorch vs TTSim: Max diff: {max_diff:.6e}, Mean diff: {mean_diff:.6e}"
            )

            if np.allclose(
                output_pt_no_mask_np, output_ttsim_no_mask_np, rtol=1e-4, atol=1e-5
            ):
        ...
        if (
            hasattr(output_ttsim_no_mask, "data")
            and output_ttsim_no_mask.data is not None
            and hasattr(output_ttsim_with_mask, "data")
            and output_ttsim_with_mask.data is not None
        ):
            ...
            if max_diff > 1e-6:
                ...
                print(
                    f"    [Warning] Masking has minimal effect (may be expected depending on mask coverage)"
                )

        # Verify shapes
        expected_shape = [bs, num_query, embed_dims]
        if (
            list(output_ttsim_no_mask.shape) == expected_shape
            and list(output_ttsim_with_mask.shape) == expected_shape
        ):
            print(
...
def test_msipm3d_construction():
    """Test MSIPM3D construction."""
    print("\n" + "=" * 80)
    print("TEST: MSIPM3D Construction")
    print("=" * 80)
    ...
    try:
        ...
        print(f"[OK] PyTorch model created: {type(model_pt).__name__}")
        print(f"[OK] TTSim model created: {type(model_tts).__name__}")

        # Check parameter counts
        pt_params = sum(p.numel() for p in model_pt.parameters())
        print(f"\nPyTorch parameters: {pt_params:,}")
        print(
            f"  - value_proj: weight={model_pt.value_proj.weight.shape}, bias={model_pt.value_proj.bias.shape}"
        )

        tts_params = model_tts.analytical_param_count()
        print(f"\nTTSim parameters: {tts_params:,}")

        if pt_params == tts_params:
            print("[OK] Parameter counts match")
        else:
            print(
                f"[X] Parameter count mismatch: PyTorch={pt_params}, TTSim={tts_params}"
            )
            all_passed = False

    except Exception as e:
        print(f"[X] MSIPM3D construction test failed: {e}")
        import traceback
...
def test_msipm3d_fixed_offsets():
    """Test that MSIPM3D fixed sampling offsets match between PyTorch and TTSim."""
    print("\n" + "=" * 80)
    print("TEST: MSIPM3D Fixed Sampling Offsets")
    print("=" * 80)
    ...
    try:
        ...
        # Compare fixed sampling offsets
        pt_offsets = model_pt.fixed_sampling_offsets.detach().cpu().numpy()
        tts_offsets = model_tts.fixed_sampling_offsets

        print(f"Fixed sampling offsets comparison:")
        print(f"  Shape: {pt_offsets.shape}")
        print(f"  Max diff: {np.abs(pt_offsets - tts_offsets).max():.6e}")
        print(f"  Mean diff: {np.abs(pt_offsets - tts_offsets).mean():.6e}")

        if np.allclose(pt_offsets, tts_offsets, rtol=1e-5, atol=1e-6):
            print("[OK] Fixed offsets match perfectly")
        else:
            print(f"[X] Fixed offsets mismatch")
            all_passed = False

    except Exception as e:
        print(f"[X] Fixed offsets test failed: {e}")
        import traceback
...
def test_msipm3d_forward():
    """Test MSIPM3D forward pass."""
    print("\n" + "=" * 80)
    print("TEST: MSIPM3D Forward Pass")
    print("=" * 80)
    ...
    try:
        ...
        bs = 2
        num_query = 4
        num_value = 16  # Total points across all levels
        embed_dims = 128
        num_heads = 4
        num_levels = 2
        num_points = 4
        num_Z_anchors = 4  # Must divide num_points
        spatial_shapes = [(4, 4), (2, 2)]  # 16 + 4 = 20 != num_value, so adjust
        num_value = 20
        ...
        weight_np = model_pt.value_proj.weight.detach().cpu().numpy()
        bias_np = model_pt.value_proj.bias.detach().cpu().numpy()
        initialize_linear_weights_with_data(model_tts.value_proj, weight_np, bias_np)

        # Create inputs
        query_np = np.random.randn(bs, num_query, embed_dims).astype(np.float32) * 0.1
        value_np = np.random.randn(bs, num_value, embed_dims).astype(np.float32) * 0.1
        reference_points_np = np.random.rand(bs, num_query, num_Z_anchors, 2).astype(
            np.float32
        )
        ...
        query_pt = torch.from_numpy(query_np)
        value_pt = torch.from_numpy(value_np)
        reference_points_pt = torch.from_numpy(reference_points_np)

        print(f"\nInput shapes:")
        print(f"  query: {query_pt.shape}")
        print(f"  value: {value_pt.shape}")
        print(f"  reference_points: {reference_points_pt.shape}")
        ...
        with torch.no_grad():
            output_pt = model_pt(
                query=query_pt,
                value=value_pt,
                reference_points=reference_points_pt,
                spatial_shapes=spatial_shapes,
            )

        print(f"\nPyTorch output: shape={output_pt.shape}")
        print(f"  range=[{output_pt.min():.4f}, {output_pt.max():.4f}]")
        print(f"  mean={output_pt.mean():.4f}, std={output_pt.std():.4f}")

        # TTSim forward pass
        query_tts = F._from_data("query", query_np, is_const=True)
        value_tts = F._from_data("value", value_np, is_const=True)
        reference_points_tts = F._from_data(
            "reference_points", reference_points_np, is_const=True
        )
        spatial_shapes_tts = F._from_data(
            "spatial_shapes", np.array(spatial_shapes, dtype=np.int64), is_const=True
        )

        output_tts = model_tts(
            query=query_tts,
            value=value_tts,
            reference_points=reference_points_tts,
            spatial_shapes=spatial_shapes_tts,
        )

        print(f"\nTTSim output: shape={output_tts.shape}")

        # Compare shapes
        if list(output_pt.shape) == list(output_tts.shape):
            print("[OK] Output shapes match")
        else:
            print(
                f"[X] Shape mismatch: PyTorch={output_pt.shape}, TTSim={output_tts.shape}"
            )
            all_passed = False

        # Compare numerical values if data is available
        if hasattr(output_tts, "data") and output_tts.data is not None:
            output_tts_np = output_tts.data
            output_pt_np = output_pt.detach().cpu().numpy()

            print(f"\nNumerical comparison:")
            print(f"  Max diff: {np.abs(output_pt_np - output_tts_np).max():.6e}")
            print(f"  Mean diff: {np.abs(output_pt_np - output_tts_np).mean():.6e}")

            # Relaxed tolerance for MSIPM3D since it has simpler structure
            if np.allclose(output_pt_np, output_tts_np, rtol=1e-3, atol=1e-4):
                print("[OK] Numerical outputs match closely")
            else:
                print(
                    f"  Note: Some numerical difference expected due to implementation details"
                )

    except Exception as e:
        print(f"[X] MSIPM3D forward pass test failed: {e}")
        import traceback
...
def main():
    ...
    results = {
        ...
        "MSIPM3D Construction": test_msipm3d_construction(),
        "MSIPM3D Fixed Offsets": test_msipm3d_fixed_offsets(),
        "MSIPM3D Forward Pass": test_msipm3d_forward(),
