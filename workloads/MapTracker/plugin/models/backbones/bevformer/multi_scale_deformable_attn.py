...
class MultiScaleDeformableAttnCore(SimNN.Module):
    ...
    def __call__(
        ...
        if isinstance(value_spatial_shapes, list):
            ...
            spatial_shapes_list = [
                (
                    int(value_spatial_shapes.data[i, 0]),
        ...
        self.stacked_flat_shape = F._from_data(
        ...
        stacked_values_flat = self.stacked_flat_op(
            stacked_values, self.stacked_flat_shape
        )
        ...
        self.attn_flat_shape = F._from_data(
...
def multi_scale_deformable_attn_ttsim(
    ...
    if parent_module is not None:
        ...
        core = MultiScaleDeformableAttnCore(name)
    return core(
        value, value_spatial_shapes, sampling_locations, attention_weights, debug
    )
...
class MultiScaleDeformableAttention(SimNN.Module):
    ...
    def __init__(
        ...
        if not batch_first:
            ...
            self.identity_transpose_out = F.Transpose(
                name + ".identity_transpose", perm=[1, 0, 2]
            )
        ...
        self.ax0_tensor = F._from_data(
            name + ".ax0", np.array([0], dtype=np.int64), is_const=True
        )
        self.ax0_2_tensor = F._from_data(
            name + ".ax0_2", np.array([0], dtype=np.int64), is_const=True
        )
        self.ax0_3_tensor = F._from_data(
            name + ".ax0_3", np.array([0], dtype=np.int64), is_const=True
        )
        self.ax_neg2_tensor = F._from_data(
            name + ".ax_neg2", np.array([-2], dtype=np.int64), is_const=True
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        sampling_offsets = self.offsets_reshape_op(
        ...
        attention_weights = self.attn_reshape_op(
            attention_weights, self.attn_shape_tensor
        )
        ...
        attention_weights = self.attn_reshape2_op(
        ...
        if reference_points.shape[-1] == 2:
            ...
            norm = self.norm_unsq1(
                self.offset_normalizer, self.ax0_tensor
            )  # (1, num_levels, 2)
            ...
            norm = self.norm_unsq4(
                norm, self.ax_neg2_tensor
