...
if not IS_POLARIS:
    ...
    from workloads.ttnn.vit.wh.vit_test_infra_polaris_wh import create_test_infra  # type: ignore[no-redef]
    from workloads.ttnn.vit.wh.vit_polaris_params_wh import config_dict
...
def test_vit_perf_device(batch_size=8, expected_kernel_samples_per_sec=1560):
    ...
    if IS_POLARIS:
        ...
        prep_device_perf_report_polaris(
            model_name=f"vit-wh-{batch_size}",
            batch_size=batch_size,
    ...
    post_processed_results = run_device_perf(
        command, subdir="vit", num_iterations=3, cols=cols, batch_size=batch_size,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run optimized sharded ViT device-perf standalone tests."
    )
    parser.add_argument(
        "test",
        nargs="?",
        ...
        help=(
            ...
            "e.g. device-ops. If omitted, runs 'device-ops'."
        ),
    )
