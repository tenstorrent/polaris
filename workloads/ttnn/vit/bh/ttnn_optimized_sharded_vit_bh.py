...
"""Polaris-side BH optimized sharded ViT.

Mirrors ``ttnn_optimized_sharded_vit_wh.py``: same SimOp graph topology,
same fused-op choices (linear+bias, linear+gelu, nlp_create_qkv_heads,
nlp_concat_heads, single softmax), same layout-TODO bookkeeping.

BH-specific differences are pushed entirely into the synthetic config
(``vit_polaris_params_bh.py``), which sets ``core_grid`` to BH 6×10=60 and
aliases ``core_grid_10x12`` for compat with the BH HW model body name.
"""
...
# untilize_with_unpadding_op is the SimOp factory (emits a UntilizeWithUnpadding
# SimOp visible in opstats); the public ttnn.untilize_with_unpadding API in
# TRACK_ONLY mode only updates a counter and does not emit a SimOp.  We need
# the SimOp form so the BH patch path matches the BH profiler trace.
from ttsim.front.ttnn.ttnn_shim import untilize_with_unpadding_op
...
_warn_layout_operator_todo_once(
    "ttnn_optimized_sharded_vit_bh_file",
...
def vit_patch_embeddings(config, pixel_values, *, parameters, unittest_check=False):
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_patch_fold_tolayout",
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_patch_proj_linear_layout",
    ...
    # BH HW emits ``UntilizeWithUnpadding`` (not plain ``Untilize``) here
    # because the matmul output is tile-padded along the row dim and the
    # post-tilize row-major view is the unpadded shape.  Polaris's matmul
    # output sets padded_shape == logical_shape, so a plain ``to_layout``
    # would route to ``untilize_op`` and we would lose parity.  Use the
    # SimOp factory directly so the trace shows ``UntilizeWithUnpadding``.
    patch_embedding_output = untilize_with_unpadding_op(
        patch_embedding_output,
        output_shape=list(patch_embedding_output.shape),
    )
...
def vit_embeddings(
    ...
    *,
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_embeddings_add_layout",
...
def vit_attention(
    ...
    *_, hidden_size = hidden_states.shape
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_attn_qkv_linear_layout",
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_attn_scores_matmul_layout",
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_attn_softmax_layout",
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_attn_context_matmul_layout",
    ...
    # NOTE: BH HW does NOT reshard between concat_heads and the output
    # projection (whereas WH HW does — and the WH polaris model keeps the
    # reshard for parity).  Profiler comparison on BH (compare_layers) showed
    # 12 extra reshard ops in the polaris trace that were absent in the BH
    # profiler trace; they were dropped to land BH at parity.  See
    # workloads/ttnn/vit/MIGRATION.md.
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_attn_output_dense_linear_layout",
...
def vit_output(
    ...
    *,
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_output_residual_add_layout",
...
def vit_layer(
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_layernorm_before_layout",
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_layer_residual_add_layout",
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_layernorm_after_layout",
...
def vit(
    ...
    _warn_layout_operator_todo_once(
        "opt_vit_bh_final_layernorm_layout",
