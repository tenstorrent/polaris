...
"""Synthetic parameter trees and config for Polaris-side BH ViT (optimized sharded).

Mirrors ``vit_polaris_params_wh.py`` (which targets Wormhole) but configures the
core grid for Blackhole.  Shared by ``run_ttnn_optimized_sharded_vit_bh.py``
and ``vit_test_infra_polaris_bh.py`` so that parameter shapes stay in sync.

Blackhole specifics (matching ``ttnn_optimized_sharded_vit_bh.py`` HW):
  - usable compute grid x=11, y=10
  - chosen grid for ViT: 6×10 = 60 cores (gcd(24, 84) constraints)
  - ``core_grid_10x12`` is an alias kept for compat with the BH model body,
    matching the HW comment "name kept for compat, now 10x6=60".
"""
...
from ttsim.front.ttnn.config import BlackholeComputeKernelConfig
...
# BH 6×10 = 60 core grid (CoreCoord(0,0) → CoreCoord(5,9))
config_obj.core_grid = ttnn.CoreGrid(
    [ttnn.CoreRange(ttnn.CoreCoord(0, 0), ttnn.CoreCoord(5, 9))]
)
# Compat alias: BH HW retained the 10x12 name but it now means 10x6=60.
config_obj.core_grid_10x12 = config_obj.core_grid
config_obj.program_configs = {
    "ln_compute_config": BlackholeComputeKernelConfig(
