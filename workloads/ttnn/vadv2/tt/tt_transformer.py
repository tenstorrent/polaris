...
from workloads.ttnn.vadv2.tt.tt_decoder import (
    TtDetectionTransformerDecoder,
    TtMapDetectionTransformerDecoder,
)
...
class TtVADPerceptionTransformer(SimNN.Module):
    ...
    def attn_bev_encode(
        ...
        **kwargs,
        ...
        bev_pos = ttnn.reshape(
            bev_pos,
            [bev_pos.shape[0], bev_pos.shape[1], bev_pos.shape[2] * bev_pos.shape[3]],
        )
        ...
        shift = ttnn.from_torch(
            shift, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT
        )  # , device=self.device)
        ...
        can_bus = ttnn.from_torch(
            can_bus, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=self.device
        )
        # [:, :]
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            if self.use_cams_embeds:
                ...
                cam_embeds = ttnn.reshape(
                    cam_embeds, (cam_embeds.shape[0], 1, 1, cam_embeds.shape[1])
                )
        ...
        spatial_shapes = ttnn.Tensor(
            ...
            device=self.device,
            data=np.array(spatial_shapes_list[0], dtype=np.uint32),
        )

        level_start_index = ttnn.zeros(
            (1,), dtype=ttnn.uint32, layout=ttnn.TILE_LAYOUT, device=self.device
        )
        feat_flatten = ttnn.permute(
            feat_flatten, (0, 2, 1, 3)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        object_query_embed = ttnn.to_layout(
            object_query_embed, layout=ttnn.ROW_MAJOR_LAYOUT
        )
        ...
        if isinstance(self.params, dict):
            ...
            reference_points = ttnn.linear(
                query_pos,
                self.params.reference_points.weight,
                bias=self.params.reference_points.bias,
            )
        ...
        map_query_pos = ttnn.Tensor(
            shape=map_query_pos.shape,
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
            device=self.device,
            data=map_query_pos.data,
        )

        map_query_pos = ttnn.to_layout(map_query_pos, layout=ttnn.TILE_LAYOUT)
        map_query = ttnn.unsqueeze(map_query, 0)
        if isinstance(self.params, dict):
            params_map_reference_points = DictAsAttr(
                self.params["map_reference_points"]
            )
            ...
            map_reference_points = ttnn.linear(
                map_query_pos,
                self.params.map_reference_points.weight,
                bias=self.params.map_reference_points.bias,
            )
        ...
        map_query = ttnn.Tensor(
            shape=map_query.shape,
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
            device=self.device,
            data=map_query.data,
