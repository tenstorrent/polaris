...
def main():
    ...
    logger.info("=" * 70)
    logger.info("  Prefix Embedding Shape Test (TTSim)")
    logger.info("=" * 70)
    ...
    try:
        ...
        # Test: embed_language
        logger.info("2. Testing embed_language...")
        lang_tokens = make_input([1, SEQ_LEN], device)
        lang_masks = make_input([1, SEQ_LEN], device)
        out = model.embed_language(lang_tokens, lang_masks)
        ...
        logger.info(f"  {status}  prefix_language_embedding  got={actual}  expected={expected}")
        logger.info("=" * 70)
