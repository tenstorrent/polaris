...
def main() -> int:
    ...
    logger.info("=" * 80)
    logger.info("  PI0 TTNN SHAPE TEST")
    logger.info("  TTSim (shape-tracking only, no numerical values)")
    logger.info("=" * 80)
    ...
    try:
        ...
        start = time.time()
        model = PI0ModelTTNN(device=device, **PI0_CONFIG)
        logger.info(f"   Model initialized in {(time.time() - start) * 1000:.2f}ms")
        ...
        logger.info(f"   Shape: {actual_shape}, Time: {(time.time() - start) * 1000:.2f}ms")
        ...
        logger.info(f"   {status}  sample_actions  got={actual_shape}  expected={EXPECTED_SHAPE}")
        logger.info("=" * 80)
