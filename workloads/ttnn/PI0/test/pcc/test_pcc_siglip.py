...
def main():
    ...
    logger.info("=" * 70)
    logger.info("  SigLIP Vision Tower Shape Test (TTSim)")
    logger.info("=" * 70)
    ...
    try:
        ...
        logger.info("2. Testing vision tower forward...")
        pixel_values = make_input([1, 3, config.image_size, config.image_size], device)
        out = model.forward(pixel_values)
        ...
        logger.info(f"  {status}  siglip_vision_tower  got={actual}  expected={expected}")
        logger.info("=" * 70)
