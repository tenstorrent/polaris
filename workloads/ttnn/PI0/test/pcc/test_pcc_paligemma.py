...
def main():
    ...
    logger.info("=" * 70)
    logger.info("  PaliGemma Backbone Shape Test (TTSim)")
    logger.info("=" * 70)
    ...
    try:
        ...
        # Test 1: embed_image
        logger.info("2. Testing embed_image...")
        pixel_values = make_input(
            [1, 3, config.siglip_config.image_size, config.siglip_config.image_size],
            device,
        )
        ...
        # Test 2: forward_vlm
        logger.info("3. Testing forward_vlm...")
        ...
        for name, actual, expected, ok in results:
            ...
            logger.info(f"  {status}  {name:<15} got={actual}  expected={expected}")
        logger.info("=" * 70)
