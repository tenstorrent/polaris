...
class SigLIPAttentionTTNN:
    def __init__(self, config, weights, device):
        ...
        if any(w is None for w in (self.q_weight, self.k_weight, self.v_weight, self.wo)):
            raise ValueError("Missing SigLIP attention projection weights for this layer")
    def forward(self, x):
        shape = x.shape
        if shape is None:
        ...
        attn = ttnn.multiply(attn, scale_t)
...
class SigLIPVisionTowerTTNN:
    ...
    def forward(self, x):
        ...
        if self.position_embedding is not None:
            ...
            if shape[0] != 1:
                pos = ttnn.repeat(pos, (shape[0], 1, 1))

            x = ttnn.add(x, pos)
            if pos is not self.position_embedding:
                ttnn.deallocate(pos)
