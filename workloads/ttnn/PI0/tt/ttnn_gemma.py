...
def rms_norm_ttnn(
    ...
    """Real RMSNorm (TTSim), implemented locally so the shared ttnn.rms_norm op
    stays unchanged (other models like llama3 rely on its existing behaviour):
        x / sqrt(mean(x^2, last-dim) + eps) * weight
    """
    sq = ttnn.multiply(x, x)
    # ttnn.mean reduces and drops the last axis; reshape restores a size-1 axis
    # so the division broadcasts back over the full width.
    ms = ttnn.mean(sq, dim=-1)
    _sh = x.shape
    if _sh is None:
        raise ValueError("rms_norm input shape must be set")
    ms = ttnn.reshape(ms, [_sh[i] for i in range(len(_sh) - 1)] + [1])
    ms = ttnn.add(ms, eps)
    rms = ttnn.sqrt(ms)
    normalized = ttnn.div(x, rms)
    return ttnn.multiply(normalized, weight)
...
def precompute_freqs_cis_ttnn(
    ...
    two_t = ttnn.full(indices_half.shape, 2.0, dtype=ttnn.float32, layout=ttnn.TILE_LAYOUT, device=device)
    indices = ttnn.multiply(indices_half, two_t)  # [0, 2, 4, ..., head_dim-2]
    ttnn.deallocate(two_t)
    ttnn.deallocate(indices_half)
    inv_head_dim_t = ttnn.full(indices.shape, 1.0 / head_dim, dtype=ttnn.float32, layout=ttnn.TILE_LAYOUT, device=device)
    exponents = ttnn.multiply(indices, inv_head_dim_t)
    ttnn.deallocate(inv_head_dim_t)
    ttnn.deallocate(indices)
    neg_log_base_t = ttnn.full(exponents.shape, -math.log(base), dtype=ttnn.float32, layout=ttnn.TILE_LAYOUT, device=device)
    neg_exp = ttnn.multiply(exponents, neg_log_base_t)
    ttnn.deallocate(neg_log_base_t)
...
class GemmaAttentionTTNN:
    ...
    def forward(
        ...
        # NOTE: ttsim's ttnn.experimental does not implement rotary_embedding, so
        # RoPE is not applied on this path (the op is unavailable). Modeling RoPE
        # faithfully in ttsim needs slice/broadcast support that isn't available
        # yet; tracked as a follow-up. Explicit no-op for now.
        q_rope = q
        k_rope = k
        ...
        try:
            ...
            attn_weights = ttnn.multiply(attn_weights, scale_t)
