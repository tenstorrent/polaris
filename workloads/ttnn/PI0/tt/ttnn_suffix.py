...
from workloads.ttnn.PI0.tt.ttnn_common import (
...
class SuffixEmbeddingTTNN:
    ...
    def __init__(
        ...
        # Attention mask is constant: [1, 1, 0, ..., 0] for PI0
        # or [1, 0, ..., 0] for PI05
        att_mask_pattern = []
        if not config.pi05:
    ...
    def embed_suffix(
        ...
        if suffix_len != self._att_mask_suffix_len:
            raise ValueError(
                f"suffix length mismatch: got {suffix_len}, expected {self._att_mask_suffix_len}"
            )

        suffix_att_masks = ttnn.repeat(self._att_mask_pattern, (batch_size, 1))
...
def convert_suffix_weights_to_ttnn(
    ...
    for key, value in ttsim_weights.items():
        if "bias" in key:
            bias_dtype = dtype or ttnn.bfloat16
            ttnn_weights[key] = tensor_1d_to_2d_ttnn(value, device, dtype=bias_dtype)
        else:
            weight_dtype = dtype or ttnn.bfloat8_b
            transposed = ttnn.typecast(ttnn.permute(value, (1, 0)), weight_dtype)
