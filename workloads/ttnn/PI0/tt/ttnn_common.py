...
def create_sinusoidal_pos_embedding_ttnn(
    ...
    exponent = ttnn.multiply(fraction, neg_log_ratio)
    ttnn.deallocate(neg_log_ratio)
    inv_period_ratio = ttnn.exp(exponent)
    scale_const = ttnn.full(
        inv_period_ratio.shape,
        (2 * math.pi) / min_period,
        dtype=ttnn.float32,
        layout=ttnn.TILE_LAYOUT,
        device=device,
    )
