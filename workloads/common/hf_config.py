...
def load_hf_config(
    ...
    *,
    ...
    """Download a JSON file from HuggingFace; fall back to a local cache.

    On a successful download the result is written to *cache_dir* so that
    subsequent calls work offline.
    ...
    try:
        ...
        logger.warning("HuggingFace download failed for {}/{}: {}", repo_id, filename, exc)
    ...
    raise RuntimeError(
        ...
        f"download from {url} failed and no cache at {cache_path}"
    )
