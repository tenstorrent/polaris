...
class RMSNorm(SimNN.Module):
    def __init__(self, objname: str, dim: int, eps: float = 1e-6):
        ...
        self.name = objname
        self.eps = eps
        self.weight = F._from_shape(f'{self.name}_weight', (dim,), is_param=True)
    ...
    def _norm(self, x):
        torchShape = x.shape
        x = torch.randn(torchShape)
        tx = x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)
        return F._from_shape(f'{self.name}_norm_tensor', tx.shape)
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    freqs_cis = torch.polar(torch.ones_like(freqs), freqs)[0:1, :]  # complex64
    freqs_shape = freqs_cis.shape
    freqs_cis = F._from_shape('freqs_cis', [*freqs_shape, 2])
    return freqs_cis
...
def apply_rotary_emb(
    ...
    xqr = xq.reshape(*xq.shape[:-1], -1, 2)
    xkr = xk.reshape(*xk.shape[:-1], -1, 2)
