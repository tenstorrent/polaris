...
class RMSNorm(SimNN.Module):
    ...
    def _norm(self, x):
        x2 = self.mulopx2(x, x)
        mu = self.meanop(x2).unsqueeze(
            -1
        )  ## y_flat.pow(2) substituted with mul ## unsqueeze for keepdim=True
...
def reshape_for_broadcast(
    freqs_cis: SimNN.SimTensor, x: SimNN.SimTensor
) -> SimNN.SimTensor:
    ndim = len(x.shape)
    assert 0 <= 1 < ndim
    assert freqs_cis.shape == [x.shape[1], x.shape[-2], x.shape[-1]]
    shape = [
        d if i == 1 or i == ndim - 2 or i == ndim - 1 else 1
...
class Attention(SimNN.Module):
    ...
    def __call__(
        ...
        scores = scores_mulout / F._from_data(
...
class FeedForward(SimNN.Module):
    ...
    def __call__(self, x):
        return self.w2(
            self.reluop(self.w1(x)) * self.w3(x)
        )  # use Relu instead of Silu - for now
...
class TransformerBlock(SimNN.Module):
    def __init__(self, objname: str, layer_id: int, args: ModelArgs):
        ...
        self.attention_norm = RMSNorm(
            f"{self.name}_attention_norm", args.dim, eps=args.norm_eps
        )
...
class Transformer(SimNN.Module):
    def __init__(self, objname: str, cfg):
        ...
        self.tok_embeddings = F.Embedding(
            f"{self.name}_tok_embeddings", self.params.vocab_size, self.params.dim
        )
        self.layers = SimNN.ModuleList(
            [
                TransformerBlock(f"{self.name}_layer_{i}", i, self.params)
                for i in range(self.n_layers)
            ]
        )
        self.norm = RMSNorm(
            f"{self.name}_rmsnorm", self.params.dim, eps=self.params.norm_eps
        )
        self.output = F.Linear(
            f"{self.name}_output", self.params.dim, self.params.vocab_size, bias=False
        )
    ...
    def __call__(self, tokens: SimNN.SimTensor = None, start_pos: int = 0):  # type: ignore[assignment]
        ...
        freqs_cis = (
            self.freqs_cis
        )  # [start_pos : start_pos + seqlen] ## assume seqlen = 1

        mask = None
        if seqlen > 1:
            raise NotImplementedError(
                "Simulation for sequence length > 1 is not implemented yet!"
            )
