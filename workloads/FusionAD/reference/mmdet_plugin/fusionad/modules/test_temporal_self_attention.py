...
from workloads.FusionAD.projects.mmdet_plugin.fusionad.modules.temporal_self_attention import TemporalSelfAttention
...
class TemporalSelfAttention_PyTorch(nn.Module):
    ...
    def __init__(self, embed_dims=256, num_heads=8, num_levels=4, num_points=4, num_bev_queue=2):
        super().__init__()
        ...
        self.sampling_offsets = nn.Linear(embed_dims * num_bev_queue,
    ...
    def forward_no_residual(self, query, value, reference_points, spatial_shapes):
        ...
        value = value.view(bs * self.num_bev_queue, num_value, self.num_heads,
        ...
        sampling_offsets = sampling_offsets.permute(0, 3, 1, 2, 4, 5, 6).reshape(
            bs * self.num_bev_queue, num_query, self.num_heads, self.num_levels,
            self.num_points, 2
        )
        ...
        reference_points_expanded = reference_points.unsqueeze(1).repeat(1, self.num_bev_queue, 1, 1, 1)
...
def test_forward_pass():
    ...
    try:
        ...
        value_np = np.random.randn(bs * num_bev_queue, num_value, num_heads, head_dim).astype(np.float32) * 0.1
        ...
        ref_points_expanded_np = np.repeat(ref_points_np, num_bev_queue, axis=0)  # [bs*num_bev_queue, nq, nl, 2]
        query_ttsim = F._from_data('query', query_np, is_const=True)
        value_ttsim = F._from_data('value', value_np, is_const=True)
        ref_points_ttsim = F._from_data('ref_points', ref_points_expanded_np, is_const=True)
...
def test_different_configurations():
    ...
    for i, (embed_dims, num_heads, num_levels, num_points, spatial_shapes) in enumerate(test_cases, 1):
        try:
            ...
            bs = 2
            num_query = sum(H * W for H, W in spatial_shapes)  # Match sum of spatial shapes
            num_bev_queue = 2
            head_dim = embed_dims // num_heads

            # Create test inputs
            query_np = np.random.randn(bs, num_query, embed_dims).astype(np.float32) * 0.1
            value_np = np.random.randn(bs * num_bev_queue, num_query, num_heads, head_dim).astype(np.float32) * 0.1
            ref_points_np = np.random.rand(bs, num_query, num_levels, 2).astype(np.float32)
            ...
            if list(output_pt.shape) == list(output_ttsim.shape):
                print(f"  [OK] Shapes match! Parameter count: {tsa.analytical_param_count():,}")
            else:
