...
from workloads.FusionAD.projects.mmdet_plugin.fusionad.modules.custom_base_transformer_layer import MyCustomBaseTransformerLayer
        ...
        for layer in module.layers:
            if hasattr(layer, 'param') and layer.param.data is None:
                ...
                if len(shape) == 2:
                    fan_in, fan_out = shape[0], shape[1]
                    limit = np.sqrt(6.0 / (fan_in + fan_out))
                    layer.param.data = np.random.uniform(-limit, limit, shape).astype(np.float32)
            if hasattr(layer, 'bias') and layer.bias is not None and layer.bias.data is None:
        ...
        if hasattr(module, attr_name):
            ...
            if module_list is not None:
                for sub_module in module_list:
                    ...
                    initialize_module_params(sub_module, seed=None)  # Don't reset seed for nested modules
...
def test_layer_norm():
    ...
    try:
        ...
        print(f"   Output range: [{output_torch_np.min():.6f}, {output_torch_np.max():.6f}]")
        ...
        match = compare_tensors(output_torch, output_ttsim, "LayerNorm output", rtol=1e-3, atol=1e-4)
...
def test_ffn():
    ...
    try:
        ...
        if hasattr(output_ttsim, 'data') and output_ttsim.data is not None:
            ...
            print(f"   TTSim output range: [{ttsim_data.min():.6f}, {ttsim_data.max():.6f}]")
        ...
        expected_params = (embed_dims * feedforward_channels + feedforward_channels +
                          feedforward_channels * embed_dims + embed_dims)
        print(f"\n4. Parameter Count:")
        print(f"   TTSim params: {param_count:,}")
        print(f"   Expected params: {expected_params:,}")
        print(f"   Match: {param_count == expected_params}")
    ...
    try:
        ...
        try:
            ...
            if hasattr(output_ttsim, 'data') and output_ttsim.data is not None:
                ...
                print(f"   [WARN] Output has no data (shape inference only - this is expected as it's a structure test)")
        except Exception as e:
...
def main():
    ...
    print("by comparing with PyTorch reference implementations and numerical validation.")
