...
"""
TTSim implementation of PansegformerHead for FusionAD panoptic segmentation.

Inference-only conversion.  Training-specific methods (loss, get_targets,
forward_train, filter_query) and post-processing (get_bboxes, forward_test)
are omitted — they operate outside the ONNX compute graph.

Architecture (forward path):
  1. Reshape bev_embed [H*W, bs, C] → [bs, C, bev_h, bev_w]
  2. Create zero masks; compute SinePositionalEncoding
  3. Transformer call → (memory_tuple, hs, init_ref, inter_refs, ...)
  4. Permute memory / query for mask decoder args
  5. Per-decoder-layer: cls_branch(hs) → cls,  reg_branch(hs) + inverse_sigmoid(ref) → sigmoid → coord
  6. Return dict(outputs_classes, outputs_coords, args_tuple, reference, ...)

Inherits from SegDETRHead (for analytical_param_count base).

Weight mapping (PyTorch -> TTSim):
  query_embedding.weight     -> query_embedding_weight (F._from_data)
  stuff_query.weight         -> stuff_query_weight (F._from_data)
  bev_embedding.weight       -> bev_embedding_weight (F._from_data)
  cls_branches[i]            -> cls_branches[i] (SimNN.Linear)
  reg_branches[i]            -> reg_branches[i] (RegBranch)
  cls_thing_branches[i]      -> cls_thing_branches[i] (SimNN.Linear)
  cls_stuff_branches[i]      -> cls_stuff_branches[i] (SimNN.Linear)
  reg_branches2[i]           -> reg_branches2[i] (RegBranch)
"""
...
# current_dir is dense_heads/ — needed for seg_head_plugin imports
...
from .seg_head_plugin.seg_deformable_transformer import SegDeformableTransformer
from .seg_head_plugin.seg_mask_head import SegMaskHead
...
# ======================================================================
# RegBranch  (num_reg_fcs=2, out_channels=4)
# ======================================================================

class RegBranch(SimNN.Module):
    """Regression branch: Linear→ReLU → Linear→ReLU → Linear→4.

    Matches PyTorch nn.Sequential(Linear, ReLU, Linear, ReLU, Linear(→4)).
    """

    def __init__(self, name, embed_dims=256, num_reg_fcs=2, out_channels=4):
        super().__init__()
        self.name = name
        ...
        # Final projection
        setattr(self, f'fc{num_reg_fcs}',
                SimNN.Linear(f'{name}.fc{num_reg_fcs}',
    ...
    def __call__(self, x):
        ...
        for i in range(self.num_reg_fcs):
            ...
            setattr(self, out.name, out)
        out = getattr(self, f'fc{self.num_reg_fcs}')(out)
        setattr(self, out.name, out)
        return out
    ...
    def analytical_param_count(self):
        ...
        for i in range(self.num_reg_fcs):
            ...
            total += fc.in_features * fc.out_features + fc.out_features
        fc_out = getattr(self, f'fc{self.num_reg_fcs}')
        total += fc_out.in_features * fc_out.out_features + fc_out.out_features
        return total
...
class PansegformerHead(SimNN.Module):
    """
    TTSim implementation of PansegformerHead (inference only).

    Args:
        name (str): Module name for TTSim graph.
        embed_dims (int): Transformer embedding dimension.
        num_query (int): Number of object queries.
        num_things_classes (int): Number of "thing" categories.
        num_stuff_classes (int): Number of "stuff" categories.
        bev_h (int): BEV grid height.
        bev_w (int): BEV grid width.
        canvas_size (tuple): Output canvas (H, W).
        num_decoder_layers (int): Number of transformer decoder layers.
        num_dec_things (int): Number of mask decoder layers for things.
        num_dec_stuff (int): Number of mask decoder layers for stuff.
        num_reg_fcs (int): Number of hidden FCs in reg branch. Default: 2.
        pos_encoding_num_feats (int): Sine positional encoding features.
        with_box_refine (bool): Whether decoder uses iterative refinement.
        as_two_stage (bool): Whether two-stage (default False).
        use_sigmoid_cls (bool): Sigmoid cls for things branches.
        transformer_cfg (dict): Config dict for SegDeformableTransformer
            (passed only when building internally).
        transformer: Pre-built SegDeformableTransformer (optional).
        things_mask_head: Pre-built SegMaskHead (optional).
        stuff_mask_head: Pre-built SegMaskHead (optional).
    """
    ...
    def __init__(self, name,
                 ...
                 num_dec_stuff=6,
                 num_reg_fcs=2,
                 pos_encoding_num_feats=128,
                 with_box_refine=True,
                 as_two_stage=False,
                 use_sigmoid_cls=True,
                 transformer_cfg=None,
                 transformer=None,
                 things_mask_head=None,
        ...
        self.num_reg_fcs = num_reg_fcs
        self.pos_encoding_num_feats = pos_encoding_num_feats
        self.with_box_refine = with_box_refine
        self.as_two_stage = as_two_stage
        ...
        elif transformer_cfg is not None:
            self.transformer = SegDeformableTransformer(
                f'{name}.transformer', **transformer_cfg)
        else:
            self.transformer = None

        # ---- Mask decoders ----
        if things_mask_head is not None:
            self.things_mask_head = things_mask_head
        else:
            self.things_mask_head = SegMaskHead(
                f'{name}.things_mask', d_model=embed_dims, nhead=8,
                num_decoder_layers=num_dec_things)

        if stuff_mask_head is not None:
            self.stuff_mask_head = stuff_mask_head
        else:
            self.stuff_mask_head = SegMaskHead(
                f'{name}.stuff_mask', d_model=embed_dims, nhead=8,
                num_decoder_layers=num_dec_stuff,
                use_self_attn=True)

        # ---- Classification branches (one Linear per decoder layer) ----
        cls_list = []
        for i in range(num_decoder_layers):
            cls_list.append(SimNN.Linear(
                f'{name}.cls_branch_{i}',
                in_features=embed_dims,
                out_features=self.cls_out_channels))
        self.cls_branches = SimNN.ModuleList(cls_list)

        # ---- Regression branches (RegBranch per decoder layer) ----
        # reg_branches: used in the head output loop for final coords
        reg_list = []
        for i in range(num_decoder_layers):
            reg_list.append(RegBranch(
                f'{name}.reg_branch_{i}',
                embed_dims=embed_dims,
                num_reg_fcs=num_reg_fcs,
                out_channels=4))
        self.reg_branches = SimNN.ModuleList(reg_list)

        # dec_reg_branches: separate copy used inside the decoder for
        # iterative reference-point refinement.  Same architecture and
        # weights as reg_branches, but distinct ONNX node names so the
        # graph stays valid when both call sites are traced.
        dec_reg_list = []
        for i in range(num_decoder_layers):
            dec_reg_list.append(RegBranch(
                f'{name}.dec_reg_branch_{i}',
                embed_dims=embed_dims,
                num_reg_fcs=num_reg_fcs,
                out_channels=4))
        self.dec_reg_branches = SimNN.ModuleList(dec_reg_list)

        # ---- Mask decoder cls/reg branches ----
        cls_thing_list = []
        for i in range(num_dec_things):
            cls_thing_list.append(SimNN.Linear(
                f'{name}.cls_thing_{i}',
                in_features=embed_dims,
                out_features=self.cls_out_channels))
        self.cls_thing_branches = SimNN.ModuleList(cls_thing_list)

        cls_stuff_list = []
        for i in range(num_dec_stuff):
            cls_stuff_list.append(SimNN.Linear(
                f'{name}.cls_stuff_{i}',
                in_features=embed_dims,
                out_features=1))
        self.cls_stuff_branches = SimNN.ModuleList(cls_stuff_list)

        reg2_list = []
        for i in range(num_dec_things):
            reg2_list.append(RegBranch(
                f'{name}.reg_branch2_{i}',
                embed_dims=embed_dims,
                num_reg_fcs=num_reg_fcs,
                out_channels=4))
        self.reg_branches2 = SimNN.ModuleList(reg2_list)

        # ---- Query embeddings (set externally before inference) ----
        self.query_embedding_weight = F._from_data(
            f'{name}.query_emb',
            np.zeros((num_query, embed_dims * 2), dtype=np.float32))

        # Stuff query and BEV embedding are used by the mask decoder path
        # which runs outside the main graph. Store as numpy to avoid
        # disconnected nodes in the ONNX graph.
        self.stuff_query_weight = np.zeros(
            (num_stuff_classes, embed_dims * 2), dtype=np.float32)

        # ---- BEV embedding (for non-two-stage mode) ----
        if not as_two_stage:
            self.bev_embedding_weight = np.zeros(
                (bev_h * bev_w, embed_dims), dtype=np.float32)

        # ---- Positional encoding (precomputed as numpy, wrapped for graph) ----
        pos_data = self._build_sine_pos_encoding(
            bev_h, bev_w, pos_encoding_num_feats,
            normalize=True, offset=-0.5, name=f'{name}.pos_enc')
        self.pos_embed = F._from_data(
            f'{name}.pos_embed', pos_data, is_const=True)

        # ---- InverseSigmoid per decoder layer ----
        self.inv_sig = []
        ...
        # ---- Reshape ops for bev_embed → [bs, C, bev_h, bev_w] ----
        self.bev_reshape = F.Reshape(f'{name}.bev_reshape')
        self.bev_perm = F.Transpose(f'{name}.bev_perm', perm=[0, 3, 1, 2])

        # ---- Per-layer add (for reference offset) and sigmoid ----
        for lvl in range(num_decoder_layers):
            setattr(self, f'ref_add_L{lvl}', F.Add(f'{name}.ref_add_L{lvl}'))
            setattr(self, f'coord_sigmoid_L{lvl}',
                    F.Sigmoid(f'{name}.coord_sig_L{lvl}'))

        # ---- Memory / query permute ops ----
        self.mem_perm = F.Transpose(f'{name}.mem_perm', perm=[1, 0, 2])
        self.query_perm = F.Transpose(f'{name}.query_perm', perm=[1, 0, 2])
        self.qpos_perm = F.Transpose(f'{name}.qpos_perm', perm=[1, 0, 2])
        self.mpos_perm = F.Transpose(f'{name}.mpos_perm', perm=[1, 0, 2])

        super().link_op2module()

    @staticmethod
    def _build_sine_pos_encoding(h, w, num_feats, normalize=True, offset=-0.5,
                                 name=None, temperature=10000.0):
    ...
    def __call__(self, bev_embed):
        """
        Forward pass of PansegformerHead.

        Args:
            bev_embed: SimTensor [H*W, bs, C] or numpy array.

        Returns:
            dict with keys:
                'bev_embed': original bev_embed (or None if two_stage)
                'outputs_classes': list of cls scores per decoder layer
                'outputs_coords': list of sigmoid bbox preds per decoder layer
                'enc_outputs_class': None (as_two_stage=False)
                'enc_outputs_coord': None (as_two_stage=False)
                'args_tuple': [memory, memory_mask, memory_pos, query, None, query_pos, hw_lvl]
                'reference': last reference tensor
        """
        n = self.name

        # ---- Extract numpy if SimTensor ----
        bev_np = bev_embed.data if hasattr(bev_embed, 'op_in') else bev_embed
        hw, bs, c = bev_np.shape

        # ---- Reshape: [H*W, bs, C] → [bs, bev_h, bev_w, C] → [bs, C, bev_h, bev_w] ----
        bev_4d_np = bev_np.transpose(1, 0, 2).reshape(
            bs, self.bev_h, self.bev_w, c).transpose(0, 3, 1, 2)
        bev_4d = F._from_data(f'{n}.bev_4d', bev_4d_np.astype(np.float32),
                              is_const=True)
        setattr(self, bev_4d.name, bev_4d)

        mlvl_feats = [bev_4d]

        # ---- Masks: all zeros [bs, bev_h, bev_w] ----
        mask_np = np.zeros((bs, self.bev_h, self.bev_w), dtype=np.float32)
        mask_t = F._from_data(f'{n}.mask', mask_np, is_const=True)
        setattr(self, mask_t.name, mask_t)
        mlvl_masks = [mask_t]

        # ---- Positional encoding [1, C, bev_h, bev_w] ----
        mlvl_pos = [self.pos_embed]

        # ---- Query embeddings ----
        query_embeds = self.query_embedding_weight

        # ---- Transformer ----
        (memory_sf, lvl_pos_sf, mask_flat, query_pos_sf), \
            hs_list, init_reference, inter_references, \
            enc_cls, enc_coord = self.transformer(
                mlvl_feats, mlvl_masks, query_embeds, mlvl_pos,
                reg_branches=self.dec_reg_branches if self.with_box_refine else None,
                cls_branches=None)

        # ---- Permute memory/query for mask decoder (swap S, B dims) ----
        # hs_list is a list of [nq, bs, C] per decoder layer

        # last layer query [nq, bs, C] → permute to [bs, nq, C]
        last_hs = hs_list[-1]
        query = self.query_perm(last_hs)
        setattr(self, query.name, query)

        # memory: [S, bs, C] → [bs, S, C]
        memory_perm = self.mem_perm(memory_sf)
        setattr(self, memory_perm.name, memory_perm)

        # query_pos: [nq, bs, C] → [bs, nq, C]
        qpos_perm = self.qpos_perm(query_pos_sf)
        setattr(self, qpos_perm.name, qpos_perm)

        # memory_pos: [S, bs, C] → [bs, S, C]
        mpos_perm = self.mpos_perm(lvl_pos_sf)
        setattr(self, mpos_perm.name, mpos_perm)

        # Convert mask_flat data for args_tuple
        mask_flat_np = _sim_data(mask_flat)

        hw_lvl = [(self.bev_h, self.bev_w)]

        args_tuple = [memory_perm, mask_flat_np, mpos_perm, query,
                      None, qpos_perm, hw_lvl]

        # ---- Per-layer cls + reg with reference refinement ----
        # Each hs_list[lvl] is a SimTensor [nq, bs, C]; permute to [bs, nq, C]
        # and feed through branches in-graph to keep connectivity.

        outputs_classes = []
        outputs_coords = []

        for lvl in range(self.num_decoder_layers):
            # Permute hidden states: [nq, bs, C] → [bs, nq, C]
            _perm_op = F.Transpose(f'{n}.hs_perm_L{lvl}', perm=[1, 0, 2])
            setattr(self, _perm_op.name, _perm_op)
            _perm_op.set_module(self)
            hs_lvl_t = _perm_op(hs_list[lvl])
            ...
            # Classification
            cls_out = self.cls_branches[lvl](hs_lvl_t)
            setattr(self, cls_out.name, cls_out)

            # Regression (uses reg_branches — distinct from dec_reg_branches
            # used inside the decoder for reference refinement)
            reg_out = self.reg_branches[lvl](hs_lvl_t)
            setattr(self, reg_out.name, reg_out)

            # Add reference offset + sigmoid (in-graph)
            # When ref is 2D, add inv_sigmoid(ref) to only first 2 channels
            inv_sig = self.inv_sig[lvl]
            inv_ref = inv_sig(reference)
            setattr(self, inv_ref.name, inv_ref)

            ref_dim = reference.shape[-1]
            ref_add = getattr(self, f'ref_add_L{lvl}')
            if ref_dim == 4:
                coord_pre = ref_add(reg_out, inv_ref)
                setattr(self, coord_pre.name, coord_pre)
            else:
                # Slice reg_out[..., :2] and reg_out[..., 2:]
                _s0 = F._from_data(f'{n}.sl_s0_L{lvl}', np.array([0], dtype=np.int64), is_const=True)
                setattr(self, _s0.name, _s0)
                _e2 = F._from_data(f'{n}.sl_e2_L{lvl}', np.array([2], dtype=np.int64), is_const=True)
                setattr(self, _e2.name, _e2)
                _e4 = F._from_data(f'{n}.sl_e4_L{lvl}', np.array([4], dtype=np.int64), is_const=True)
                setattr(self, _e4.name, _e4)
                _ax = F._from_data(f'{n}.sl_ax_L{lvl}', np.array([-1], dtype=np.int64), is_const=True)
                setattr(self, _ax.name, _ax)
                sl_xy = F.SliceF(f'{n}.sl_reg_xy_L{lvl}')
                setattr(self, sl_xy.name, sl_xy); sl_xy.set_module(self)
                sl_wh = F.SliceF(f'{n}.sl_reg_wh_L{lvl}')
                setattr(self, sl_wh.name, sl_wh); sl_wh.set_module(self)

                reg_xy = sl_xy(reg_out, _s0, _e2, _ax)
                setattr(self, reg_xy.name, reg_xy)
                reg_wh = sl_wh(reg_out, _e2, _e4, _ax)
                setattr(self, reg_wh.name, reg_wh)

                reg_xy_ref = ref_add(reg_xy, inv_ref)
                setattr(self, reg_xy_ref.name, reg_xy_ref)

                cat_op = F.ConcatX(f'{n}.cat_reg_L{lvl}', axis=-1)
                setattr(self, cat_op.name, cat_op); cat_op.set_module(self)
                coord_pre = cat_op(reg_xy_ref, reg_wh)
                setattr(self, coord_pre.name, coord_pre)

            coord_sig = getattr(self, f'coord_sigmoid_L{lvl}')
            coord_out = coord_sig(coord_pre)
            setattr(self, coord_out.name, coord_out)

            outputs_classes.append(_sim_data(cls_out))
            outputs_coords.append(_sim_data(coord_out))
        ...
        # Last reference
        last_ref = inter_references[-1] if len(inter_references) > 0 else init_reference
        last_ref_np = _sim_data(last_ref)

        # ---- Mask decoder path (things + stuff) ----
        # Mirrors original get_bboxes: feeds query subsets through
        # things_mask_head and stuff_mask_head to produce segmentation masks.
        # For graph construction we use a fixed number of synthetic thing
        # queries rather than the dynamic top-k selection in the original.
        num_thing_queries = min(self.num_query, 100)  # fixed graph size

        # thing_query: first num_thing_queries of the last-layer query
        # query is [bs, nq, C] (already permuted above)
        thing_sl = F.SliceF(f'{n}.thing_sl')
        setattr(self, thing_sl.name, thing_sl); thing_sl.set_module(self)
        _s0t = F._from_data(f'{n}.thing_s0', np.array([0], dtype=np.int64), is_const=True)
        setattr(self, _s0t.name, _s0t)
        _e_th = F._from_data(f'{n}.thing_end', np.array([num_thing_queries], dtype=np.int64), is_const=True)
        setattr(self, _e_th.name, _e_th)
        _ax1 = F._from_data(f'{n}.thing_ax', np.array([1], dtype=np.int64), is_const=True)
        setattr(self, _ax1.name, _ax1)

        thing_query = thing_sl(query, _s0t, _e_th, _ax1)  # [bs, num_thing_queries, C]
        setattr(self, thing_query.name, thing_query)

        # stuff_query: learnable embedding [1, num_stuff_classes, C]
        stuff_q_emb = self.stuff_query_weight[:, :self.embed_dims]  # numpy
        stuff_q_t = F._from_data(f'{n}.stuff_q_emb',
                                  np.tile(stuff_q_emb[np.newaxis], (bs, 1, 1)).astype(np.float32),
                                  is_param=True)
        setattr(self, stuff_q_t.name, stuff_q_t)

        stuff_qpos_emb = self.stuff_query_weight[:, self.embed_dims:]  # numpy
        stuff_qpos_t = F._from_data(f'{n}.stuff_qpos_emb',
                                     np.tile(stuff_qpos_emb[np.newaxis], (bs, 1, 1)).astype(np.float32),
                                     is_param=True)
        setattr(self, stuff_qpos_t.name, stuff_qpos_t)

        # memory_perm is [bs, S, C] from above; mask_flat_np is numpy
        mask_flat_t = F._from_data(f'{n}.mask_flat',
                                    mask_flat_np.astype(np.float32) if isinstance(mask_flat_np, np.ndarray) else np.zeros((bs, hw), dtype=np.float32),
                                    is_const=True)
        setattr(self, mask_flat_t.name, mask_flat_t)

        # Things mask head: query=thing_query, key/value=memory, no pos_query
        mask_things, mask_inter_things, query_inter_things = self.things_mask_head(
            memory_perm, mask_flat_t, None,
            thing_query, None, None,
            hw_lvl=hw_lvl)

        # Stuff mask head: query=stuff_q, key/value=memory, pos_query=stuff_qpos
        mask_stuff, mask_inter_stuff, query_inter_stuff = self.stuff_mask_head(
            memory_perm, mask_flat_t, None,
            stuff_q_t, None, stuff_qpos_t,
            hw_lvl=hw_lvl)

        # Stuff classification from last inter-layer query
        stuff_cls_out = self.cls_stuff_branches[-1](query_inter_stuff[-1])
        setattr(self, stuff_cls_out.name, stuff_cls_out)
        stuff_cls_sig = F.Sigmoid(f'{n}.stuff_cls_sig')
        setattr(self, stuff_cls_sig.name, stuff_cls_sig); stuff_cls_sig.set_module(self)
        stuff_scores = stuff_cls_sig(stuff_cls_out)
        setattr(self, stuff_scores.name, stuff_scores)

        # Concatenate thing + stuff masks: [bs, N_thing+N_stuff, L, 1]
        mask_cat = F.ConcatX(f'{n}.mask_cat', axis=1)
        setattr(self, mask_cat.name, mask_cat); mask_cat.set_module(self)
        all_masks = mask_cat(mask_things, mask_stuff)
        setattr(self, all_masks.name, all_masks)

        return {
            'bev_embed': None if self.as_two_stage else bev_embed,
            'outputs_classes': all_cls,
            'outputs_coords': all_coords,
            'enc_outputs_class': None,
            'enc_outputs_coord': None,
            'args_tuple': args_tuple,
            'reference': last_ref_np,
            'mask_things': mask_things,
            'mask_stuff': mask_stuff,
            'stuff_scores': stuff_scores,
            'all_masks': all_masks,
        }

    def analytical_param_count(self, lvl=0):
        """Total learnable parameters in PansegformerHead."""
        indent = "  " * lvl
        total = 0

        # cls_branches: num_decoder_layers × Linear(embed_dims, cls_out_channels)
        for i in range(self.num_decoder_layers):
            fc = self.cls_branches[i]
            c = fc.in_features * fc.out_features + fc.out_features
            total += c

        # reg_branches: num_decoder_layers × RegBranch
        for i in range(self.num_decoder_layers):
            total += self.reg_branches[i].analytical_param_count()

        # cls_thing_branches: num_dec_things × Linear(embed_dims, cls_out_channels)
        for i in range(self.num_dec_things):
            fc = self.cls_thing_branches[i]
            c = fc.in_features * fc.out_features + fc.out_features
            total += c

        # cls_stuff_branches: num_dec_stuff × Linear(embed_dims, 1)
        for i in range(self.num_dec_stuff):
            fc = self.cls_stuff_branches[i]
            c = fc.in_features * fc.out_features + fc.out_features
            total += c

        # reg_branches2: num_dec_things × RegBranch
        for i in range(self.num_dec_things):
            total += self.reg_branches2[i].analytical_param_count()

        # query_embedding: num_query × 2 * embed_dims
        total += self.num_query * self.embed_dims * 2

        # stuff_query: num_stuff_classes × 2 * embed_dims
        total += self.num_stuff_classes * self.embed_dims * 2

        # bev_embedding (if not two_stage)
        if not self.as_two_stage:
            total += self.bev_h * self.bev_w * self.embed_dims

        # transformer params
        if self.transformer is not None and hasattr(self.transformer, 'analytical_param_count'):
            total += self.transformer.analytical_param_count()

        # mask heads
        if hasattr(self.things_mask_head, 'analytical_param_count'):
            total += self.things_mask_head.analytical_param_count()
        if hasattr(self.stuff_mask_head, 'analytical_param_count'):
            total += self.stuff_mask_head.analytical_param_count()

        if lvl >= 1:
            logger.debug(f"{indent}Total PansegformerHead params: {total:,}")
...
if __name__ == '__main__':
    logger.info("panseg_head.py — TTSim Module Tests")
    logger.info("=" * 70)
    ...
    # ------------------------------------------------------------------
    # Test 1: RegBranch construction + forward
    # ------------------------------------------------------------------
    try:
        rb = RegBranch('test_rb', embed_dims=embed_dims)
        x_np = np.random.randn(1, num_query, embed_dims).astype(np.float32)
        x_t = F._from_data('rb_in', x_np)
        out = rb(x_t)
        assert list(out.shape) == [1, num_query, 4], \
            f"Expected [1, {num_query}, 4], got {list(out.shape)}"
        logger.debug(f"  [Test 1] RegBranch: PASS — shape={list(out.shape)}")
        passed += 1
    except Exception as e:
        logger.debug(f"  [Test 1] RegBranch: FAIL — {e}")
        failed += 1

    # ------------------------------------------------------------------
    # Test 2: PansegformerHead construction
    # ------------------------------------------------------------------
    try:
        ...
        checks = [
            ('cls_branches', len(head.cls_branches) == num_dec_layers),
            ('reg_branches', len(head.reg_branches) == num_dec_layers),
            ('cls_thing_branches', len(head.cls_thing_branches) == num_dec_things),
            ('cls_stuff_branches', len(head.cls_stuff_branches) == num_dec_stuff),
            ('reg_branches2', len(head.reg_branches2) == num_dec_things),
            ('query_embedding_weight', hasattr(head, 'query_embedding_weight')),
            ('stuff_query_weight', hasattr(head, 'stuff_query_weight')),
            ('pos_embed', hasattr(head, 'pos_embed')),
            ('things_mask_head', hasattr(head, 'things_mask_head')),
            ('stuff_mask_head', hasattr(head, 'stuff_mask_head')),
        ]
        ...
        if all_ok:
            logger.debug(f"  [Test 2] Construction: PASS — all attributes present")
            passed += 1
        else:
            logger.debug(f"  [Test 2] Construction: FAIL")
            failed += 1
    except Exception as e:
        logger.debug(f"  [Test 2] Construction: FAIL — {e}")
        failed += 1

    # ------------------------------------------------------------------
    # Test 3: SinePositionalEncoding
    # ------------------------------------------------------------------
    try:
        pe = PansegformerHead._build_sine_pos_encoding(bev_h, bev_w,
                                                        embed_dims // 2)
        expected = (1, embed_dims, bev_h, bev_w)
        assert pe.shape == expected, \
            f"Expected {expected}, got {pe.shape}"
        assert pe.dtype == np.float32
        logger.debug(f"  [Test 3] SinePositionalEncoding: PASS — shape={pe.shape}")
        passed += 1
    except Exception as e:
        logger.debug(f"  [Test 3] SinePositionalEncoding: FAIL — {e}")
        failed += 1

    # ------------------------------------------------------------------
    # Test 4: RegBranch param count
    # ------------------------------------------------------------------
    try:
        rb2 = RegBranch('test_rb2', embed_dims=256, num_reg_fcs=2,
                         out_channels=4)
        count = rb2.analytical_param_count()
        # 2 hidden: (256*256+256)*2 = 131,584
        # final: 256*4+4 = 1,028
        # total: 132,612
        expected_count = (256 * 256 + 256) * 2 + (256 * 4 + 4)
        assert count == expected_count, \
            f"Expected {expected_count}, got {count}"
        logger.debug(f"  [Test 4] RegBranch param count: PASS — {count:,}")
        passed += 1
    except Exception as e:
        logger.debug(f"  [Test 4] RegBranch param count: FAIL — {e}")
        failed += 1

    # ------------------------------------------------------------------
    # Test 5: Graph connectivity (RegBranch output)
    # ------------------------------------------------------------------
    try:
        assert hasattr(out, 'op_in'), "RegBranch output not a SimTensor"
        logger.debug(f"  [Test 5] Graph connectivity: PASS")
        passed += 1
    except Exception as e:
        logger.debug(f"  [Test 5] Graph connectivity: FAIL — {e}")
        failed += 1
