...
"""
TTSim implementation of Spatial Cross Attention.
...
1. Initialization Functions (mmcv.cnn):
   - xavier_init: Implemented in init_utils.py using numpy (production)
   - constant_init: Implemented in init_utils.py using numpy (production)
...
try:
    ...
    # Handle case when run as script or from test files
...
try:
    ...
    # Handle case when run as script or from test files
    from workloads.FusionAD.projects.mmdet_plugin.fusionad.modules.init_utils import xavier_init, constant_init, _is_power_of_2
...
class SpatialCrossAttention(SimNN.Module):
    ...
    def __init__(self,
        ...
        self.value_transpose_op = F.Transpose(name + '.value_transpose', perm=[2, 0, 1, 3])
        self.value_reshape_op = F.Reshape(name + '.value_reshape')

        # Query expansion
        self.queries_expand = F.Unsqueeze(name + '.queries_expand')
        self.queries_expand_axis = F._from_data(name + '.queries_expand_axis', np.array([1], dtype=np.int64), is_const=True)
        self.queries_tile = F.Tile(name + '.queries_tile')
        self.queries_flat = F.Reshape(name + '.queries_flat')

        # Reference points
        self.ref_points_transpose = F.Transpose(name + '.ref_points_transpose', perm=[1, 0, 2, 3, 4])
        ...
        # BEV mask ops
        # bev_mask is expected as [num_cams, bs, nq] (already reduced by encoder)
        self.bev_mask_transpose = F.Transpose(name + '.bev_mask_transpose', perm=[1, 0, 2])
        self.bev_mask_expand = F.Unsqueeze(name + '.bev_mask_expand')
        self.bev_mask_expand_axis = F._from_data(name + '.bev_mask_expand_axis', np.array([3], dtype=np.int64), is_const=True)
        ...
        self.count_expand_axis = F._from_data(name + '.count_expand_axis', np.array([2], dtype=np.int64), is_const=True)
        ...
        if not batch_first:
            self.output_transpose_back = F.Transpose(name + '.output_transpose_back', perm=[1, 0, 2])
    ...
    def __call__(self,
        ...
        _t = F._from_data(self.name + '.key_reshape_shape',
        ...
        _t = F._from_data(self.name + '.value_reshape_shape',
                        np.array([bs * self.num_cams, l, self.embed_dims], dtype=np.int64),
                        is_const=True)
        setattr(self, _t.name, _t)
        ...
        _t = F._from_data(self.name + '.queries_tile_reps',
        ...
        _t = F._from_data(self.name + '.queries_flat_shape',
        ...
        _t = F._from_data(self.name + '.ref_points_flat_shape',
                        np.array([bs * self.num_cams, num_query, D, 2], dtype=np.int64),
                        is_const=True)
        setattr(self, _t.name, _t)
        reference_points_flat_out = self.ref_points_flat(reference_points_transposed, _t)
        ...
        # Reshape output back
        _t = F._from_data(self.name + '.queries_out_reshape_shape',
                        np.array([bs, self.num_cams, num_query, self.embed_dims], dtype=np.int64),
                        is_const=True)
        setattr(self, _t.name, _t)
...
class MSDeformableAttention3D(SimNN.Module):
    ...
    def __init__(self,
        ...
        if not batch_first:
            self.query_transpose = F.Transpose(name + '.query_transpose', perm=[1, 0, 2])
            self.value_transpose = F.Transpose(name + '.value_transpose', perm=[1, 0, 2])
            self.output_transpose = F.Transpose(name + '.output_transpose', perm=[1, 0, 2])

        # Mask ops
        self.mask_expand = F.Unsqueeze(name + '.mask_expand')
        self.mask_expand_axis = F._from_data(name + '.mask_expand_axis', np.array([2], dtype=np.int64), is_const=True)
        ...
        self.ref_ax2 = F._from_data(name + '.ref_ax2', np.array([2], dtype=np.int64), is_const=True)
        self.ref_ax3 = F._from_data(name + '.ref_ax3', np.array([3], dtype=np.int64), is_const=True)
        self.ref_ax4 = F._from_data(name + '.ref_ax4', np.array([4], dtype=np.int64), is_const=True)
        ...
        self.norm_ax0 = F._from_data(name + '.norm_ax0', np.array([0], dtype=np.int64), is_const=True)
        self.norm_ax0_2 = F._from_data(name + '.norm_ax0_2', np.array([0], dtype=np.int64), is_const=True)
        self.norm_ax0_3 = F._from_data(name + '.norm_ax0_3', np.array([0], dtype=np.int64), is_const=True)
        self.norm_ax0_4 = F._from_data(name + '.norm_ax0_4', np.array([4], dtype=np.int64), is_const=True)
    ...
    def __call__(self,
        ...
        if key_padding_mask is not None:
            ...
            zero_value = F._from_data(self.name + '.zero_value',
                                      np.zeros(value.shape, dtype=np.float32), is_const=True)
        ...
        _t = F._from_data(self.name + '.offsets_shape',
        ...
        _t = F._from_data(self.name + '.attn_shape',
        ...
        _t = F._from_data(self.name + '.attn_shape2',
                    np.array([bs, num_query, self.num_heads, self.num_levels, self.num_points], dtype=np.int64),
                    is_const=True)
        setattr(self, _t.name, _t)
        ...
        if reference_points.shape[-1] == 2:
            ...
            if isinstance(spatial_shapes, list):
                ...
                if hasattr(spatial_shapes, 'data'):
                    ...
                    shapes_np = np.array([[H, W] for H, W in spatial_shapes], dtype=np.float32)
                offset_normalizer_data = np.stack([shapes_np[:, 1], shapes_np[:, 0]], axis=-1).astype(np.float32)
            ...
            _t = F._from_data(self.name + '.offsets_z_shape',
                        np.array([bs, num_query, self.num_heads, self.num_levels, num_points_per_anchor, num_Z_anchors, 2], dtype=np.int64),
                        is_const=True)
            setattr(self, _t.name, _t)
            ...
            sampling_locations = self.sampling_locs_add(ref_pts, normalized_offsets_reshaped)
            setattr(self, sampling_locations.name, sampling_locations)

            # Reshape back
            _t = F._from_data(self.name + '.sampling_locs_shape',
                        np.array([bs, num_query, self.num_heads, self.num_levels, self.num_points, 2], dtype=np.int64),
                        is_const=True)
            setattr(self, _t.name, _t)
        ...
        output = multi_scale_deformable_attn_ttsim(
            self.name + '.msda',
            value,
            spatial_shapes if isinstance(spatial_shapes, list) else [(int(spatial_shapes.data[i, 0]), int(spatial_shapes.data[i, 1])) for i in range(self.num_levels)],
            sampling_locations,
            attention_weights,
            parent_module=self
        )
