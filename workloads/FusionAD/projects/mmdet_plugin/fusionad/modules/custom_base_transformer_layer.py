...
class MyCustomBaseTransformerLayer(SimNN.Module):
    ...
    def __init__(self,
        ...
        assert set(operation_order) & valid_ops == set(operation_order), \
    ...
    def __call__(self,
        ...
        if attn_masks is None:
            ...
            # If single mask provided (e.g. a tensor), replicate for all attentions
            attn_masks = [
                copy.deepcopy(attn_masks) for _ in range(self.num_attn)
            ]
            warnings.warn(f'Use same attn_mask in all attentions in '
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                query = self.attentions[attn_index](
                    ...
                    temp_key,
                    temp_value,
                    identity=identity if self.pre_norm else query,
                    query_pos=query_pos,
                    key_pos=query_pos,
                    attn_mask=attn_masks[attn_index],
                    key_padding_mask=query_key_padding_mask,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.debug("  [OK] MyCustomBaseTransformerLayer constructed successfully")
        ...
        logger.debug(f"  [X] MyCustomBaseTransformerLayer construction failed: {e}")
        import traceback
        traceback.print_exc()
