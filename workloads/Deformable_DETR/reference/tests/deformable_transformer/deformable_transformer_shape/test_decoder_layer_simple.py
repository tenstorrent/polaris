...
"""
Simple test for DeformableTransformerDecoderLayer.
...
def test_decoder_layer():
    ...
    print("\n" + "=" * 80)
    print("TEST: DeformableTransformerDecoderLayer")
    print("=" * 80)
    print("\nObjective: Validate SHAPE + NUMERICAL computation")
    print("What we compute:")
    print("  1. Self-attention: Q/K/V projections + attention scores + weighted sum")
    print("  2. Cross-attention: Deformable attention on encoder memory")
    print("  3. FFN: Linear → ReLU → Linear")
    print("Output: Refined query features [batch, num_queries, d_model]\n")
    ...
    src_spatial_shapes = [[10, 10], [7, 7], [5, 5], [2, 13]]  # 100+49+25+26=200
    ...
    reference_points_torch = torch.rand(batch_size, num_queries, n_levels, 2)
    ...
    layer_pytorch = DecoderLayerPyTorch(
        d_model=d_model,
        d_ffn=1024,
        dropout=0.1,
        activation="relu",
