...
"""
CombConvLayer module validation: ttsim vs PyTorch comparison.
"""
...
class CombConvLayerPytorch(nn.Sequential):
    def __init__(
...
# ttsim module
from workloads.ScaledYOLOv4.models.common import CombConvLayer as CombConvLayerTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (all parameters in CombConvLayer)
    weight_bytes = 0
    ...
    # Layer1: ConvLayer (conv + bn + relu6)
    conv1 = pytorch_module.layer1.conv
    ...
    relu1_ops = 1 * C_out1 * H * W  # ReLU6 is just comparison + clamp
    total_operations += conv1_ops + bn1_ops + relu1_ops

    # Layer2: DWConvLayer (depthwise conv + bn)
    dwconv = pytorch_module.layer2.dwconv
    K_dw = dwconv.kernel_size[0] * dwconv.kernel_size[1]
    C_dw = dwconv.groups  # depthwise means groups = channels
    # Depthwise conv: each output channel only convolves with one input channel
    stride = dwconv.stride[0]
    H_out = (H + 2 * dwconv.padding[0] - K_dw) // stride + 1
    W_out = (W + 2 * dwconv.padding[1] - K_dw) // stride + 1
    dwconv_ops = 2 * K_dw * C_dw * H_out * W_out  # 2 for MAC, but only per-channel
    bn2_ops = 4 * C_dw * H_out * W_out
    total_operations += dwconv_ops + bn2_ops
...
def validate_comb_conv_layer_module(verbose: bool = False):
    """Validate CombConvLayer module by comparing PyTorch output with ttsim .data output."""
    print("\n-- CombConvLayer Module Validation --")
    ...
    print(
        f"Input: {input_shape}, Config: in={in_channels}, out={out_channels}, k={kernel}, s={stride}"
    )

    pytorch_module = CombConvLayerPytorch(
        in_channels, out_channels, kernel=kernel, stride=stride
    )
    pytorch_module.eval()
    ...
    module_name = "test_combconv"
    ttsim_module = CombConvLayerTtsim(
        module_name, in_channels, out_channels, kernel=kernel, stride=stride
    )
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("CombConvLayer Module Validation")
    print("=" * 60 + "\n")

    result = validate_comb_conv_layer_module()
