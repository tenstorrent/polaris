...
"""
Conv module validation: ttsim vs PyTorch comparison.
"""
...
def inject_pytorch_weights_to_ttsim(
    ...
    """Inject PyTorch weights into ttsim module's parameter tensors."""
...
def debug_tensors(ttsim_module):
    """Print debug info about all tensors in a ttsim module."""
...
class ConvPytorch(nn.Module):
    ...
    def __init__(
        ...
        super(ConvPytorch, self).__init__()
...
# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import Conv as ConvTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (conv weights + bn parameters)
    weight_bytes = 0
    ...
    # Conv2d operations: 2 * K * K * C_in * C_out * H_out * W_out (MAC = 2 ops)
    ...
    conv_ops = 2 * K * C_in * C_out * H_out * W_out
    total_operations += conv_ops

    # BatchNorm operations: 4 ops per element (subtract, divide, multiply, add)
    bn_ops = 4 * output.numel()
    total_operations += bn_ops

    # Activation (Mish) operations: ~5 ops per element (softplus, tanh, multiply)
    act_ops = 5 * output.numel()
    total_operations += act_ops
...
def validate_conv_module(verbose: bool = False):
    """Validate Conv module by comparing PyTorch output with ttsim .data output."""
    print("\n-- Conv Module Validation --")
    ...
    print(
        f"Input: {input_shape}, Config: c1={in_channels}, c2={out_channels}, k={kernel_size}, s={stride}"
    )

    # PyTorch module
    pytorch_conv = ConvPytorch(
    ...
    input_torch = torch.from_numpy(input_data)
    with torch.no_grad():
        pytorch_output = pytorch_conv(input_torch)
    ...
    inject_pytorch_weights_to_ttsim(pytorch_conv, ttsim_conv, prefix=module_name)

    output_tensor = ttsim_conv(input_tensor)

    # Validate output correctness
    if output_tensor.data is not None:
        ttsim_output = output_tensor.data
        result = compare_outputs(
            pytorch_output_np, ttsim_output, "Conv", input_data, ttsim_conv, verbose
        )
    else:
        print("FAIL: ttsim output has no data")
        debug_tensors(ttsim_conv)
        return {
            "passed": False,
            "input_shape": input_shape,
    ...
    try:
        ...
        pytorch_mem_stats = calculate_pytorch_memory_stats(
            pytorch_conv, input_data, iterations=100
        )

        # Calculate ttsim memory stats
        ttsim_mem_stats = calculate_ttsim_memory_stats(ttsim_conv, device)
        ...
        # Suppress warning output to keep terminal clean. Attach error info
        # to the result so callers can inspect it if needed.
        err_info = {"error": str(e)}
        result.setdefault("memory_stats", {})["error"] = err_info
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Conv Module Validation")
    print("=" * 60 + "\n")

    result = validate_conv_module()
