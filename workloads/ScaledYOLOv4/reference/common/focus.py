...
"""
Focus module validation: ttsim vs PyTorch comparison.
"""
...
class ConvPytorch(nn.Module):
    """Conv module matching ttsim Conv structure."""
...
class FocusPytorch(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__()
        self.conv = ConvPytorch(c1 * 4, c2, k, s, p, g, act)

    def forward(self, x):
        # Checkerboard slicing
        s1 = x[:, :, ::2, ::2]
        s2 = x[:, :, 1::2, ::2]
        s3 = x[:, :, ::2, 1::2]
        s4 = x[:, :, 1::2, 1::2]
        concatenated = torch.cat([s1, s2, s3, s4], dim=1)
        return self.conv(concatenated)


# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import Focus as FocusTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Focus: slice ops (minimal), concat, conv
    total_operations = 0

    # Conv2d operations in the embedded Conv module
    conv = pytorch_module.conv.conv
    K = conv.kernel_size[0] * conv.kernel_size[1]
    C_in = conv.in_channels  # = c1 * 4 after concat
...
def validate_focus_module(verbose: bool = False):
    """Validate Focus module by comparing PyTorch output with ttsim .data output."""
    ...
    print(f"Input: {input_shape}, Config: c1={c1}, c2={c2}, k={k}, s={s}")

    # PyTorch
    pytorch_module = FocusPytorch(c1, c2, k=k, s=s, act=True)
    pytorch_module.eval()
    ...
    # ttsim
    module_name = "test_focus"
    ttsim_module = FocusTtsim(module_name, c1, c2, k=k, s=s, act="Mish")
    ...
    if output_tensor.data is not None:
        ...
        result = compare_outputs(
            pytorch_output_np, ttsim_output, "Focus", input_data, ttsim_module, verbose
        )
    else:
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Focus Module Validation")
    print("=" * 60 + "\n")

    result = validate_focus_module()
