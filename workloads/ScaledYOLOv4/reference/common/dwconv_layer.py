...
"""
DWConvLayer module validation: ttsim vs PyTorch comparison.
"""
...
class DWConvLayerPytorch(nn.Sequential):
...
# ttsim module
from workloads.ScaledYOLOv4.models.common import DWConvLayer as DWConvLayerTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (all parameters in DWConvLayer)
    weight_bytes = 0
    ...
    # DWConvLayer: Depthwise Conv2d + BatchNorm
    total_operations = 0
    ...
    dwconv = pytorch_module.dwconv
    K = dwconv.kernel_size[0] * dwconv.kernel_size[1]
    C = dwconv.groups  # depthwise means groups = channels
    stride = dwconv.stride[0]
    padding = dwconv.padding[0]

    # Output dimensions
    H_out = (H + 2 * padding - dwconv.kernel_size[0]) // stride + 1
    W_out = (W + 2 * padding - dwconv.kernel_size[1]) // stride + 1

    # Depthwise convolution: each output channel only convolves with one input channel
    # Operations: K * C * H_out * W_out (2 ops per MAC)
    dwconv_ops = 2 * K * C * H_out * W_out
    total_operations += dwconv_ops

    # BatchNorm: 4 operations per element (subtract mean, divide by std, scale, shift)
    bn_ops = 4 * C * H_out * W_out
...
def calculate_ttsim_memory_stats(ttsim_module, device):
    ...
    for op_name, op in ops_dict.items():
        ...
        if op.uses_compute_pipe is None:
            # Conv uses matrix pipe, BN uses vector pipe
...
def validate_dwconv_layer_module(verbose: bool = False):
    """Validate DWConvLayer module by comparing PyTorch output with ttsim .data output."""
    print("\n-- DWConvLayer Module Validation --")
    ...
    print(
        f"Input: {input_shape}, Config: in={in_channels}, out={out_channels}, stride={stride}"
    )

    pytorch_module = DWConvLayerPytorch(in_channels, out_channels, stride=stride)
    pytorch_module.eval()
    ...
    ttsim_module = DWConvLayerTtsim(
        module_name, in_channels, out_channels, stride=stride
    )
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("DWConvLayer Module Validation")
    print("=" * 60 + "\n")

    result = validate_dwconv_layer_module()
