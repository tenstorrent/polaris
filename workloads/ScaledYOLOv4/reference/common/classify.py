...
"""
Classify module validation: ttsim vs PyTorch comparison.
"""
...
class ClassifyPytorch(nn.Module):
...
# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import Classify as ClassifyTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Classify: AdaptiveAvgPool + Conv + Flatten
    total_operations = 0

    # AdaptiveAvgPool: average pooling ops
    pool_ops = (
        input_data.shape[1] * input_data.shape[2] * input_data.shape[3]
    )  # C * H * W averages
    total_operations += pool_ops

    # Conv operations after pooling (on 1x1 spatial)
...
def validate_classify_module(verbose: bool = False):
    """Validate Classify module by comparing PyTorch output with ttsim .data output."""
    print("\n-- Classify Module Validation --")
    ...
    # PyTorch
    pytorch_module = ClassifyPytorch(c1, c2, k=k)
    pytorch_module.eval()
    ...
    # ttsim
    module_name = "test_classify"
    ttsim_module = ClassifyTtsim(module_name, c1, c2, k=k)
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Classify Module Validation")
    print("=" * 60 + "\n")

    result = validate_classify_module()
