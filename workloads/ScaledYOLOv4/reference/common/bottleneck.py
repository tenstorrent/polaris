...
"""
Bottleneck module validation: ttsim vs PyTorch comparison.
"""
...
class BottleneckPytorch(nn.Module):
    ...
    def __init__(
        ...
        super(BottleneckPytorch, self).__init__()
...
# ttsim module
from workloads.ScaledYOLOv4.models.common import Bottleneck as BottleneckTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (all parameters in bottleneck: cv1 and cv2 conv+bn)
    weight_bytes = 0
    ...
    H_out1 = output.shape[2]
    W_out1 = output.shape[3]
    conv1_ops = 2 * K1 * C_in1 * C_out1 * H_out1 * W_out1
    total_operations += conv1_ops

    # cv1 BatchNorm + Activation
    bn1_ops = 4 * C_out1 * H_out1 * W_out1
    act1_ops = 5 * C_out1 * H_out1 * W_out1
    total_operations += bn1_ops + act1_ops

    # cv2: Conv2d 3x3
    conv2 = pytorch_module.cv2.conv
    K2 = conv2.kernel_size[0] * conv2.kernel_size[1]
    C_in2 = conv2.in_channels
    C_out2 = conv2.out_channels
    H_out2 = output.shape[2]
    W_out2 = output.shape[3]
    conv2_ops = 2 * K2 * C_in2 * C_out2 * H_out2 * W_out2
    total_operations += conv2_ops

    # cv2 BatchNorm + Activation
    bn2_ops = 4 * C_out2 * H_out2 * W_out2
    act2_ops = 5 * C_out2 * H_out2 * W_out2
    total_operations += bn2_ops + act2_ops

    # Residual add (if applicable)
    if pytorch_module.add:
        add_ops = output.numel()
...
def validate_bottleneck_module(verbose: bool = False):
    """Validate Bottleneck module by comparing PyTorch output with ttsim .data output."""
    print("\n-- Bottleneck Module Validation --")
    ...
    print(f"Input: {input_shape}, Config: c1={c1}, c2={c2}, shortcut={shortcut}, e={e}")

    pytorch_module = BottleneckPytorch(c1, c2, shortcut=shortcut, g=1, e=e)
    pytorch_module.eval()
    ...
    module_name = "test_bottleneck"
    ttsim_module = BottleneckTtsim(module_name, c1, c2, shortcut=shortcut, g=1, e=e)
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Bottleneck Module Validation")
    print("=" * 60 + "\n")

    result = validate_bottleneck_module()
