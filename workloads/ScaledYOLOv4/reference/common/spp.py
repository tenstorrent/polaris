...
"""
SPP module validation: ttsim vs PyTorch comparison.
"""
...
class SPPPytorch(nn.Module):
    def __init__(self, c1, c2, k=(5, 9, 13)):
        super().__init__()
        c_ = c1 // 2
        self.cv1 = ConvPytorch(c1, c_, 1, 1)
        self.cv2 = ConvPytorch(c_ * (len(k) + 1), c2, 1, 1)
...
# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import SPP as SPPTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # SPP: cv1 conv, multiple maxpools, concat, cv2 conv
    total_operations = 0
    ...
    # Intermediate shape after cv1
    H_int = input_data.shape[2]
    W_int = input_data.shape[3]
    conv1_ops = 2 * K1 * C_in1 * C_out1 * H_int * W_int
    bn1_ops = 4 * C_out1 * H_int * W_int
    mish1_ops = 5 * C_out1 * H_int * W_int
    total_operations += conv1_ops + bn1_ops + mish1_ops

    # MaxPool operations for each pool (comparison ops)
    num_pools = len(pytorch_module.m)
    for m in pytorch_module.m:
        k = m.kernel_size
        maxpool_ops = C_out1 * H_int * W_int * k * k
        total_operations += maxpool_ops

    # cv2: Conv2d + BN + Mish
    conv2 = pytorch_module.cv2.conv
    K2 = conv2.kernel_size[0] * conv2.kernel_size[1]
    C_in2 = conv2.in_channels  # = c_ * (len(k) + 1)
    C_out2 = conv2.out_channels
    H_out = output.shape[2]
    W_out = output.shape[3]
    conv2_ops = 2 * K2 * C_in2 * C_out2 * H_out * W_out
    bn2_ops = 4 * output.numel()
    mish2_ops = 5 * output.numel()
    total_operations += conv2_ops + bn2_ops + mish2_ops
...
def validate_spp_module(verbose: bool = False):
    """Validate SPP module by comparing PyTorch output with ttsim .data output."""
    print("\n-- SPP Module Validation --")
    ...
    # PyTorch
    pytorch_module = SPPPytorch(c1, c2, k=k)
    pytorch_module.eval()
    ...
    # ttsim
    module_name = "test_spp"
    ttsim_module = SPPTtsim(module_name, c1, c2, k=k)
    ...
    if output_tensor.data is not None:
        ...
        result = compare_outputs(
            pytorch_output_np, ttsim_output, "SPP", input_data, ttsim_module, verbose
        )
    else:
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("SPP Module Validation")
    print("=" * 60 + "\n")

    result = validate_spp_module()
