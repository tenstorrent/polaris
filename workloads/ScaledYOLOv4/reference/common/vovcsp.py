...
"""
VoVCSP module validation: ttsim vs PyTorch comparison.
"""
...
class VoVCSPPytorch(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = ConvPytorch(c1 // 2, c_, 3, 1)
        self.cv2 = ConvPytorch(c_, c_, 3, 1)
        self.cv3 = ConvPytorch(c_ * 2, c2, 1)
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Calculate total operations for PyTorch VoVCSP module
    ...
    total_operations = 0
    c_ = int(c_out * 0.5)  # e=0.5 by default

    # Estimate operations for all Conv layers + BN + activations
    # cv1-cv3 convolutions + BN + Mish
    # This is a rough estimate based on module structure
    total_operations = c_out * h_out * w_out * c_in * 50  # Simplified
...
# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import VoVCSP as VoVCSPTtsim


def validate_vovcsp_module(verbose: bool = False):
    """Validate VoVCSP module by comparing PyTorch output with ttsim .data output."""
    print("\n-- VoVCSP Module Validation --")

    batch_size = 1
    c1, c2 = 64, 64  # c1 must be even for chunk
    height, width = 16, 16

    input_shape = [batch_size, c1, height, width]
    input_data = np.random.randn(*input_shape).astype(np.float32)

    print(f"Input: {input_shape}, Config: c1={c1}, c2={c2}")

    # PyTorch
    pytorch_module = VoVCSPPytorch(c1, c2)
    pytorch_module.eval()
    ...
    # ttsim
    module_name = "test_vovcsp"
    ttsim_module = VoVCSPTtsim(module_name, c1, c2)
    ...
    if output_tensor.data is not None:
        ...
        result = compare_outputs(
            pytorch_output_np, ttsim_output, "VoVCSP", input_data, ttsim_module, verbose
        )
    else:
