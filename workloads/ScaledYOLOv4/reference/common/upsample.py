...
"""
Upsample module validation: ttsim vs PyTorch comparison.
Compares nn.Upsample (PyTorch) with ttsim Upsample module.
"""
...
# ttsim Upsample module
from workloads.ScaledYOLOv4.models.common import Upsample as UpsampleTtsim


def calculate_pytorch_memory_stats(
    pytorch_module, input_data, scale_factor, iterations=100
):
    ...
    # Upsample has no trainable parameters
    weight_bytes = 0
    ...
    # Upsample (nearest) is essentially memory operations - no compute
    # Each output element requires reading one input element (nearest neighbor)
    # Operations: read input element + write output element
    total_operations = output.numel()  # Number of output elements to generate
...
def calculate_ttsim_memory_stats(ttsim_module, device):
    ...
    for op_name, op in ops_dict.items():
        ...
        if op.uses_compute_pipe is None:
            # Upsample is a memory operation, uses vector pipe
...
def validate_upsample_module(verbose: bool = False):
    """Validate Upsample module by comparing PyTorch nn.Upsample with ttsim Upsample."""
    print("\n-- Upsample Module Validation --")
    ...
    print(f"Input: {input_shape}, scale_factor={scale_factor}, mode='{mode}'")
    print(f"Expected output: {expected_output_shape}")

    # PyTorch nn.Upsample
    pytorch_module = nn.Upsample(scale_factor=scale_factor, mode=mode)
    pytorch_module.eval()
    ...
    # ttsim Upsample
    module_name = "test_upsample"
    ttsim_module = UpsampleTtsim(
        module_name, size=None, scale_factor=scale_factor, mode=mode
    )

    input_tensor = create_input_tensor_with_data("input", input_shape, input_data)

    # Upsample has no trainable parameters, so no weight injection needed
    ...
    if output_tensor.data is not None:
        ...
        # For Upsample, we may only have shape inference without data propagation
        # Check if shape matches at minimum
        print(f"\n-- Shape Validation (no data propagation) --")
        print(f"PyTorch output shape: {list(pytorch_output_np.shape)}")
        print(f"ttsim output shape:   {output_tensor.shape}")

        input_values = truncate_values(input_data)
        pytorch_values = truncate_values(pytorch_output_np)
        shape_match = list(pytorch_output_np.shape) == output_tensor.shape
        if shape_match:
            print(f"Result: PASS (shape match)")
            result = {
                "passed": True,
                "input_shape": input_shape,
                "input_values": input_values,
                ...
                "ttsim_output_values": "N/A",
                "shape_only": True,
            }
        else:
            print(f"Result: FAIL (shape mismatch)")
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Upsample Module Validation")
    print("=" * 60 + "\n")

    result_nearest = validate_upsample_module()

    print("\n" + "=" * 60)
    if result_nearest.get("passed", False):
        print(f"Output Validation: PASS")
    else:
        print(f"Output Validation: FAIL")

    if "memory_stats" in result_nearest:
