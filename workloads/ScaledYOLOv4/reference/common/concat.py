...
"""
Concat module validation: ttsim vs PyTorch comparison.
"""
...
# ------------------- PyTorch----------------------#
# Concat uses torch.cat directly, no class needed

# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import Concat as ConcatTtsim


def calculate_pytorch_memory_stats(input_data, iterations=100):
    """Calculate memory performance metrics for PyTorch concat operation."""
    # For concat, input_data is just the first tensor for measurement purposes
    input1_torch = torch.from_numpy(input_data)

    # Warmup - create dummy second tensor with same shape
    dummy_input2 = torch.randn_like(input1_torch)
    with torch.no_grad():
        for _ in range(10):
            _ = torch.cat([input1_torch, dummy_input2], dim=1)

    # Measure execution time
    start_time = time.perf_counter()
    with torch.no_grad():
        for _ in range(iterations):
            output = torch.cat([input1_torch, dummy_input2], dim=1)
    ...
    input_bytes = input_data.nbytes * 2  # Approximate for 2 inputs
    weight_bytes = 0  # Concat has no weights
    ...
    # Concat is essentially zero operations (just memory reorganization)
    total_operations = 0
...
def validate_concat_module(verbose: bool = False):
    """Validate Concat module by comparing PyTorch output with ttsim .data output."""
    print("\n-- Concat Module Validation --")

    batch_size = 1
    height, width = 16, 16
    c1, c2, c3 = 16, 32, 24  # Different channel sizes

    input1_shape = [batch_size, c1, height, width]
    input2_shape = [batch_size, c2, height, width]
    input3_shape = [batch_size, c3, height, width]

    input1_data = np.random.randn(*input1_shape).astype(np.float32)
    input2_data = np.random.randn(*input2_shape).astype(np.float32)
    input3_data = np.random.randn(*input3_shape).astype(np.float32)

    # Combined input for reporting
    combined_input = input1_data  # Just use first input for stats

    print(f"Inputs: {input1_shape}, {input2_shape}, {input3_shape}")
    print(f"Expected output: [{batch_size}, {c1+c2+c3}, {height}, {width}]")

    # PyTorch
    input1_torch = torch.from_numpy(input1_data)
    input2_torch = torch.from_numpy(input2_data)
    input3_torch = torch.from_numpy(input3_data)

    with torch.no_grad():
        pytorch_output = torch.cat([input1_torch, input2_torch, input3_torch], dim=1)
    pytorch_output_np = pytorch_output.detach().cpu().numpy()

    # ttsim
    module_name = "test_concat"
    ttsim_module = ConcatTtsim(module_name, dimension=1)

    input1_tensor = create_input_tensor_with_data("input1", input1_shape, input1_data)
    input2_tensor = create_input_tensor_with_data("input2", input2_shape, input2_data)
    input3_tensor = create_input_tensor_with_data("input3", input3_shape, input3_data)

    output_tensor = ttsim_module([input1_tensor, input2_tensor, input3_tensor])
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Concat Module Validation")
    print("=" * 60 + "\n")

    result = validate_concat_module()
