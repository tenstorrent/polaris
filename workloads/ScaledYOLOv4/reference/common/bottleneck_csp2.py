...
"""
BottleneckCSP2 module validation: ttsim vs PyTorch comparison.
"""
...
class BottleneckCSP2Pytorch(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(
        self, c1, c2, n=1, shortcut=False, g=1, e=0.5
    ):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(BottleneckCSP2Pytorch, self).__init__()
...
# ttsim module
from workloads.ScaledYOLOv4.models.common import BottleneckCSP2 as BottleneckCSP2Ttsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (all parameters in BottleneckCSP2)
    weight_bytes = 0
    ...
    # cv2: Conv2d 1x1 (on x1, no bn, no act)
    conv2_ops = (
    ...
    # cv3: Conv (1x1 conv + bn + mish)
    conv3 = pytorch_module.cv3.conv
    K3 = conv3.kernel_size[0] * conv3.kernel_size[1]
    C_in3 = conv3.in_channels
    C_out3 = conv3.out_channels
    conv3_ops = 2 * K3 * C_in3 * C_out3 * H * W
    bn3_ops = 4 * C_out3 * H * W
    act3_ops = 5 * C_out3 * H * W
    total_operations += conv3_ops + bn3_ops + act3_ops
...
def validate_bottleneck_csp2_module(verbose: bool = False):
    """Validate BottleneckCSP2 module by comparing PyTorch output with ttsim .data output."""
    print("\n-- BottleneckCSP2 Module Validation --")
    ...
    print(f"Input: {input_shape}, Config: c1={c1}, c2={c2}, n={n}, shortcut={shortcut}")

    pytorch_module = BottleneckCSP2Pytorch(c1, c2, n=n, shortcut=shortcut, g=1, e=0.5)
    pytorch_module.eval()
    ...
    module_name = "test_csp2"
    ttsim_module = BottleneckCSP2Ttsim(
        module_name, c1, c2, n=n, shortcut=shortcut, g=1, e=0.5
    )
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("BottleneckCSP2 Module Validation")
    print("=" * 60 + "\n")

    result = validate_bottleneck_csp2_module()
