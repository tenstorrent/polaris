...
"""
ConvLayer module validation: ttsim vs PyTorch comparison.
"""
...
class ConvLayerPytorch(nn.Sequential):
    def __init__(
...
# ttsim module
from workloads.ScaledYOLOv4.models.common import ConvLayer as ConvLayerTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (all parameters in ConvLayer)
    weight_bytes = 0
    ...
    # ConvLayer: Conv2d + BatchNorm + ReLU6
    total_operations = 0
    ...
    # Convolution operations: 2 operations per MAC (multiply + accumulate)
    ...
    # ReLU6 activation: comparison + clamp ~= 1 op per element
    relu6_ops = 1 * C_out * H_out * W_out
    total_operations += relu6_ops
...
def validate_conv_layer_module(verbose: bool = False):
    """Validate ConvLayer module by comparing PyTorch output with ttsim .data output."""
    print("\n-- ConvLayer Module Validation --")
    ...
    pytorch_module = ConvLayerPytorch(
    ...
    module_name = "test_convlayer"
    ttsim_module = ConvLayerTtsim(
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("ConvLayer Module Validation")
    print("=" * 60 + "\n")

    result = validate_conv_layer_module()
