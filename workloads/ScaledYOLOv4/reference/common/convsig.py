...
"""
ConvSig (Conv + Sigmoid) module validation: ttsim vs PyTorch comparison.
"""
...
class ConvSigPytorch(nn.Module):
...
# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import ConvSig as ConvSigTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Sigmoid operations: ~4 ops per element (negate, exp, add, reciprocal)
    act_ops = 4 * output.numel()
...
def calculate_ttsim_memory_stats(ttsim_module, device):
    ...
    for op_name, op in ops_dict.items():
        if op.uses_compute_pipe is None:
            # Conv uses matrix pipe, activation uses vector pipe
...
def validate_convsig_module(verbose: bool = False):
    """Validate ConvSig module by comparing PyTorch output with ttsim .data output."""
    print("\n-- ConvSig Module Validation --")
    ...
    # PyTorch
    pytorch_module = ConvSigPytorch(c1, c2, k=k, s=s)
    pytorch_module.eval()
    ...
    # ttsim
    module_name = "test_convsig"
    ttsim_module = ConvSigTtsim(module_name, c1, c2, k=k, s=s)
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("ConvSig Module Validation")
    print("=" * 60 + "\n")

    result = validate_convsig_module()
