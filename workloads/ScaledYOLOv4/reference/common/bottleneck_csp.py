...
"""
BottleneckCSP module validation: ttsim vs PyTorch comparison.
"""
...
def build_name_mapping(pytorch_name: str) -> str:
    ...
    name = re.sub(r"^bn\.weight$", "bn.scale", name)
    name = re.sub(r"\.norm\.weight$", ".norm.scale", name)
    name = re.sub(r"^norm\.weight$", "norm.scale", name)
    ...
    if ".conv.weight" in name:
        ...
        name = name.replace(".dwconv.weight", ".dwconv.param")
    elif name.startswith("dwconv.weight"):
        name = name.replace("dwconv.weight", "dwconv.param")
    elif (
...
class BottleneckCSPPytorch(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(
        self, c1, c2, n=1, shortcut=True, g=1, e=0.5
    ):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(BottleneckCSPPytorch, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
...
# ttsim module
from workloads.ScaledYOLOv4.models.common import BottleneckCSP as BottleneckCSPTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (all parameters in BottleneckCSP)
    weight_bytes = 0
    ...
    # Calculate total operations for PyTorch module
    total_operations = 0

    # cv1: Conv (1x1 conv + bn + mish)
    conv1 = pytorch_module.cv1.conv
    K1 = conv1.kernel_size[0] * conv1.kernel_size[1]
    C_in1 = conv1.in_channels
    C_out1 = conv1.out_channels
    H = input_data.shape[2]
    W = input_data.shape[3]
    conv1_ops = 2 * K1 * C_in1 * C_out1 * H * W
    bn1_ops = 4 * C_out1 * H * W
    act1_ops = 5 * C_out1 * H * W
    total_operations += conv1_ops + bn1_ops + act1_ops

    # m: Sequential bottleneck modules
    n = len(pytorch_module.m)  # number of bottleneck modules
    for bottleneck in pytorch_module.m:
        # Each bottleneck has cv1 (1x1) and cv2 (3x3)
        # cv1
        conv_b1 = bottleneck.cv1.conv
        Kb1 = conv_b1.kernel_size[0] * conv_b1.kernel_size[1]
        C_inb1 = conv_b1.in_channels
        C_outb1 = conv_b1.out_channels
        conv_b1_ops = 2 * Kb1 * C_inb1 * C_outb1 * H * W
        bn_b1_ops = 4 * C_outb1 * H * W
        act_b1_ops = 5 * C_outb1 * H * W
        total_operations += conv_b1_ops + bn_b1_ops + act_b1_ops

        # cv2
        conv_b2 = bottleneck.cv2.conv
        Kb2 = conv_b2.kernel_size[0] * conv_b2.kernel_size[1]
        C_inb2 = conv_b2.in_channels
        C_outb2 = conv_b2.out_channels
        conv_b2_ops = 2 * Kb2 * C_inb2 * C_outb2 * H * W
        bn_b2_ops = 4 * C_outb2 * H * W
        act_b2_ops = 5 * C_outb2 * H * W
        total_operations += conv_b2_ops + bn_b2_ops + act_b2_ops

        # Residual add if applicable
        if bottleneck.add:
            add_ops = C_outb2 * H * W
            total_operations += add_ops

    # cv3: Conv2d 1x1 (no bn, no act)
    conv3_ops = (
        2 * 1 * pytorch_module.cv3.in_channels * pytorch_module.cv3.out_channels * H * W
    )
    total_operations += conv3_ops

    # cv2: Conv2d 1x1 (no bn, no act)
    conv2_ops = (
        2 * 1 * pytorch_module.cv2.in_channels * pytorch_module.cv2.out_channels * H * W
    )
    total_operations += conv2_ops

    # Concat operation (no ops, just memory)

    # bn: BatchNorm on concatenated (2 * c_)
    bn_concat_channels = pytorch_module.bn.num_features
    bn_concat_ops = 4 * bn_concat_channels * H * W
    total_operations += bn_concat_ops

    # act: Mish
    act_concat_ops = 5 * bn_concat_channels * H * W
    total_operations += act_concat_ops

    # cv4: Conv (1x1 conv + bn + mish)
    conv4 = pytorch_module.cv4.conv
    K4 = conv4.kernel_size[0] * conv4.kernel_size[1]
    C_in4 = conv4.in_channels
    C_out4 = conv4.out_channels
    conv4_ops = 2 * K4 * C_in4 * C_out4 * H * W
    bn4_ops = 4 * C_out4 * H * W
    act4_ops = 5 * C_out4 * H * W
    total_operations += conv4_ops + bn4_ops + act4_ops
...
def validate_bottleneck_csp_module(verbose: bool = False):
    """Validate BottleneckCSP module by comparing PyTorch output with ttsim .data output."""
    print("\n-- BottleneckCSP Module Validation --")
    ...
    print(f"Input: {input_shape}, Config: c1={c1}, c2={c2}, n={n}")

    pytorch_module = BottleneckCSPPytorch(c1, c2, n=n, shortcut=True, g=1, e=0.5)
    pytorch_module.eval()
    ...
    module_name = "test_csp"
    ttsim_module = BottleneckCSPTtsim(
        module_name, c1, c2, n=n, shortcut=True, g=1, e=0.5
    )
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("BottleneckCSP Module Validation")
    print("=" * 60 + "\n")

    result = validate_bottleneck_csp_module()
