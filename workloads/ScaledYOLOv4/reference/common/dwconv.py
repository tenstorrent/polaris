...
"""
DWConv function validation: ttsim vs PyTorch comparison.
DWConv is a function that returns a Conv with groups=gcd(c1, c2).
"""
...
def DWConvPytorch(c1, c2, k=1, s=1, act=True):
    # Depthwise convolution
    return Conv(c1, c2, k, s, g=math.gcd(c1, c2), act=act)


# ------------------- ttsim----------------------#
from workloads.ScaledYOLOv4.models.common import DWConv as DWConvTtsim
...
def calculate_pytorch_memory_stats(pytorch_module, input_data, iterations=100):
    ...
    # Weight data (all parameters in DWConv)
    weight_bytes = 0
    ...
    # DWConv is Conv with groups=gcd(c1, c2)
    total_operations = 0
    ...
    # Output dimensions
    H_out = (H + 2 * padding - conv.kernel_size[0]) // stride + 1
    W_out = (W + 2 * padding - conv.kernel_size[1]) // stride + 1

    # Grouped convolution: each group processes (C_in/groups) input channels
    # to produce (C_out/groups) output channels
    conv_ops = 2 * K * (C_in // groups) * C_out * H_out * W_out
    total_operations += conv_ops

    # BatchNorm: 4 operations per element (subtract mean, divide by std, scale, shift)
    bn_ops = 4 * C_out * H_out * W_out
    total_operations += bn_ops

    # Mish activation: softplus + tanh + multiply
    # softplus: log(1 + exp(x)) ~= 3 ops
    # tanh: ~2 ops
    # multiply: 1 op
    # Total: ~6 ops per element
    mish_ops = 6 * C_out * H_out * W_out
    total_operations += mish_ops
...
def validate_dwconv_module(verbose: bool = False):
    """Validate DWConv function by comparing PyTorch output with ttsim .data output."""
    print("\n-- DWConv Function Validation --")

    batch_size = 1
    in_channels = 64
    out_channels = 64  # For DWConv, typically c1 == c2
    kernel_size = 3
    stride = 1
    height, width = 16, 16
    ...
    print(
        f"Input: {input_shape}, Config: c1={in_channels}, c2={out_channels}, k={kernel_size}, s={stride}, groups={groups}"
    )

    # PyTorch DWConv returns a Conv module with groups=gcd(c1, c2)
    pytorch_module = DWConvPytorch(
        in_channels, out_channels, k=kernel_size, s=stride, act=True
    )
    pytorch_module.eval()
    ...
    # ttsim DWConv also returns a Conv module with groups=gcd(c1, c2)
    # ttsim Conv expects act='Mish' (string) instead of act=True (boolean)
    module_name = "test_dwconv"
    ttsim_module = DWConvTtsim(
        module_name, in_channels, out_channels, k=kernel_size, s=stride, act="Mish"
    )
    ...
    if output_tensor.data is not None:
        ...
        result = compare_outputs(
            pytorch_output_np, ttsim_output, "DWConv", input_data, ttsim_module, verbose
        )
    else:
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("DWConv Function Validation")
    print("=" * 60 + "\n")

    result = validate_dwconv_module()
